...
// @function                A helper function for use in `upAndDownVolumeCalc()` and `upAndDownVolumeTickCalc()`. It
//                          classifies the bar's data as positive or negative based on the values of `open` and `close`.
//                          The volume calculation functions use this helper to categorize volume data when they cannot
//                          directly determine the category using `bid` and `ask` values.
...
// @function                A helper function for use in `requestUpAndDownVolume()` and `requestVolumeDelta()`. It
//                          categorizes the volume of a tick-timeframe bar as positive (up) or negative (down).
//                          The function uses `bid` and `ask` values when available and falls back to `priceCalc()`
//                          when it cannot directly determine the volume category.
// @returns                 ([float, float, float]) A tuple containing the following values:
//                              - The positive (up) volume, or 0 if the volume is negative.
//                              - The negative (down) volume, expressed as a negative quantity, or 0 if the volume is
//                                positive.
//                              - The polarized volume value.
upAndDownVolumeTickCalc() =>
    float posVol = 0.0
    float negVol = 0.0
    var bool isBuyVolume = close >= open
    float avg = math.avg(ask, bid)
...
// @function                Calculates the cumulative sum of values in an array, starting from the first element, and
//                          tracks the highest and lowest sums achieved throughout the accumulation. The function
//                          returns `na` values until it encounters the first nonempty array. Afterward, an empty array
//                          returns 0 for all values.
// @param arr               (array<float>) The array of numeric values to analyze.
// @returns                 ([float, float, float]) A tuple containing the following:
//                              - The highest cumulative sum achieved during the calculation.
//                              - The lowest cumulative sum achieved during the calculation.
//                              - The final cumulative sum after adding all element values.
arrayHighLowCalc(array<float> arr) =>
    var bool useNa = true
    if useNa and arr.size() > 0
        useNa := false
    float last = useNa ? na : 0.0
    float max  = useNa ? na : 0.0
    float min  = useNa ? na : 0.0
    for item in arr
        last := nz(last) + item
        max  := math.max(nz(max), last)
        min  := math.min(nz(min), last)
    [max, min, last]
...
// @param lowerTimeframe    (series string) The timeframe of the requested intrabar data. Accepts a valid time-based or
//                          tick-based timeframe string (e.g., "1T", "10T", "5", "15", "1D"), an empty string,
//                          `timeframe.period`, or `timeframe.main_period`. Smaller timeframes provide higher precision
//                          but cover fewer bars on the script's main timeframe. Larger timeframes cover more bars on
//                          the script's main timeframe but offer less granularity. If the specified timeframe is higher
//                          than the script's main timeframe, it causes a runtime error.
// @returns                 ([float, float, float]) A tuple containing the following values:
...
export requestUpAndDownVolume(series string lowerTimeframe) =>
    ...
    bool isTickTimeframe = str.endswith(lowerTimeframe, "T") or
         (str.length(str.trim(lowerTimeframe)) == 0 and timeframe.isticks)
    [tickPosVol, tickNegVol, tickVolume] = upAndDownVolumeTickCalc()
    [posVol, negVol, volumeDelta, hiVol, loVol] = upAndDownVolumeCalc()
    switch
        isTickTimeframe =>
            [posVolArray, negVolArray, _] = request.security_lower_tf(
                syminfo.tickerid, lowerTimeframe, [tickPosVol, tickNegVol, tickVolume],
                calc_bars_count = 200000
            )
            float requestedPosVol = posVolArray.sum()
            float requestedNegVol = negVolArray.sum()
            [requestedPosVol, requestedNegVol, requestedPosVol + requestedNegVol]
        =>
            [requestedPosVol, requestedNegVol, requestedDelta, _, _] = request.security(
                syminfo.tickerid, lowerTimeframe, [posVol, negVol, volumeDelta, hiVol, loVol],
                calc_bars_count = 200000
            )
            [requestedPosVol, requestedNegVol, requestedDelta]
...
export requestVolumeDelta(series string lowerTimeframe, series string cumulativePeriod = "") =>
    ...
    bool isTickTimeframe = str.endswith(lowerTimeframe, "T") or
         (str.length(str.trim(lowerTimeframe)) == 0 and timeframe.isticks)
    [_, _, tickVolume] = upAndDownVolumeTickCalc()
    [posVol, negVol, volumeDelta, hiVol, loVol] = upAndDownVolumeCalc()
    [delta, maxVolume, minVolume] = switch
        isTickTimeframe =>
            diffVolArray = request.security_lower_tf(
                syminfo.tickerid, lowerTimeframe, tickVolume, calc_bars_count = 200000
            )
            [arrayMaxVolume, arrayMinVolume, arrayLastVolume] = arrayHighLowCalc(diffVolArray)
            [arrayLastVolume, arrayMaxVolume, arrayMinVolume]
        =>
            [_, _, requestedDelta, requestedMaxVolume, requestedMinVolume] = request.security(
                syminfo.tickerid, lowerTimeframe, [posVol, negVol, volumeDelta, hiVol, loVol],
                calc_bars_count = 200000
            )
            [requestedDelta, requestedMaxVolume, requestedMinVolume]
