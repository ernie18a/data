<!-- tradingview-pine-id: PUB;19b5574bd1a045b68f53a21e9460c406 -->
<!-- tradingviewscripts-format: 1 -->
# Central Bank Liquidity Gap Indicator

Source: https://www.tradingview.com/script/udTI2IUw-Central-Bank-Liquidity-Gap-Indicator/

## Description

This indicator measures the gap between global liquidity growth and stock market growth to identify potential buying opportunities.
...
// ║  and selected index growth to identify potential market turning points.       ║
// ║                                                                               ║
...
string comparison_ticker = input.string("SP:SPX", title="Comparison Ticker", group="General Settings", tooltip="Ticker to compare against global liquidity (default: S&P 500)")
...
bool use_india_m2 = input.bool(false, "India M2", group="Tier 2 - Other Major Economies", tooltip="Reserve Bank of India M2 Money Supply")
...
// ============================================
bool show_correlation = input.bool(true, "Show Correlation Stats", group="Correlation Analysis", tooltip="Display rolling and lead correlation between liquidity and comparison ticker")
...
int lead_periods = input.int(12, "Lead Periods", minval=1, maxval=52, group="Correlation Analysis", tooltip="Number of bars to shift for lead correlation (tests if liquidity predicts ticker)")
...
// ============================================
// Fetch real-time exchange rates from forex markets
// Using FX_IDC data feed (Forex interbank rates)
float eurusd_rate = request.security("FX_IDC:EURUSD", timeFrame, close, ignore_invalid_symbol=true)
float gbpusd_rate = request.security("FX_IDC:GBPUSD", timeFrame, close, ignore_invalid_symbol=true)
float usdjpy_rate = request.security("FX_IDC:USDJPY", timeFrame, close, ignore_invalid_symbol=true)
float usdcad_rate = request.security("FX_IDC:USDCAD", timeFrame, close, ignore_invalid_symbol=true)
float usdchf_rate = request.security("FX_IDC:USDCHF", timeFrame, close, ignore_invalid_symbol=true)
float usdcny_rate = request.security("FX_IDC:USDCNY", timeFrame, close, ignore_invalid_symbol=true)
float usdinr_rate = request.security("FX_IDC:USDINR", timeFrame, close, ignore_invalid_symbol=true)

// ============================================
// CURRENCY CONVERSION FACTORS (Local Currency to USD)
// ============================================
// Calculate conversion factors from live forex rates with fallback to hardcoded values
float USD_FACTOR = 1.0
float CNY_FACTOR = usdcny_rate > 0 ? 1.0 / usdcny_rate : 0.14       // Chinese Yuan to USD (inverted from USDCNY)
float EUR_FACTOR = eurusd_rate > 0 ? eurusd_rate : 1.08             // Euro to USD
float JPY_FACTOR = usdjpy_rate > 0 ? 1.0 / usdjpy_rate : 0.0067     // Japanese Yen to USD (inverted from USDJPY)
float GBP_FACTOR = gbpusd_rate > 0 ? gbpusd_rate : 1.27             // British Pound to USD
float CAD_FACTOR = usdcad_rate > 0 ? 1.0 / usdcad_rate : 0.74       // Canadian Dollar to USD (inverted from USDCAD)
float CHF_FACTOR = usdchf_rate > 0 ? 1.0 / usdchf_rate : 1.13       // Swiss Franc to USD (inverted from USDCHF)
float INR_FACTOR = usdinr_rate > 0 ? 1.0 / usdinr_rate : 0.012      // Indian Rupee to USD (inverted from USDINR)
...
float india_m2_raw = request.security("ECONOMICS:INM2", timeFrame, close, ignore_invalid_symbol=true)
...
// ============================================
float ticker_close = request.security(comparison_ticker, timeFrame, close)
...
float india_m2_usd = nz(india_m2_raw, 0) * INR_FACTOR
...
float india_weight = use_india_m2 ? india_m2_usd / total_m2_usd : 0.0
...
float india_m2_growth = use_india_m2 ? calcGrowth(india_m2_raw, india_m2_raw[lookbackPeriod]) : 0.0
...
// Comparison Ticker Growth
float ticker_growth = calcGrowth(ticker_close, ticker_close[lookbackPeriod])
...
// ============================================
float money_flow_divergence = final_liquidity_growth - ticker_growth
...
// ============================================
// Rolling Correlation: How correlated are liquidity growth and ticker growth over the lookback period
float rolling_corr = ta.correlation(final_liquidity_growth, ticker_growth, corr_lookback)

// Lead Correlation: Does liquidity growth predict future ticker growth?
// Correlate past liquidity growth with current ticker growth
float lead_corr = ta.correlation(final_liquidity_growth[lead_periods], ticker_growth, corr_lookback)
...
plot(show_component_lines and use_india_m2 ? india_m2_growth : na, "India M2 Growth", color=#4CAF50, linewidth=2)   // Green
...
// WEIGHTS TABLE
// ============================================
// Extract ticker symbol for display (remove exchange prefix if present)
array<string> ticker_parts = str.split(comparison_ticker, ":")
string ticker_display = array.size(ticker_parts) > 1 ? array.get(ticker_parts, 1) : comparison_ticker
...
if show_weights_table
    ...
    if barstate.islast
        ...
        // India M2
        if use_india_m2
            table.cell(weights_table, 0, row, " India M2 ", bgcolor=row_bg, text_color=color.white, text_halign=text.align_left, text_size=font_size)
            table.cell(weights_table, 1, row, str.tostring(india_weight * 100, "#.#") + "% ", bgcolor=row_bg, text_color=color.white, text_halign=text.align_right, text_size=font_size)
            row += 1
        ...
        if show_correlation
            ...
            // Rolling Correlation - measures how liquidity and ticker move together now
            table.cell(weights_table, 0, row, " Corr w/ " + ticker_display + " (" + str.tostring(corr_lookback) + "b) ", bgcolor=row_bg, text_color=color.white, text_halign=text.align_left, text_size=font_size)
            ...
            // Lead Correlation - measures if past liquidity predicts current ticker
            table.cell(weights_table, 0, row, " Leads " + ticker_display + " by " + str.tostring(lead_periods) + "b ", bgcolor=row_bg, text_color=color.white, text_halign=text.align_left, text_size=font_size)
...
// ALERTS
// ============================================
alertcondition(ta.crossover(money_flow_divergence, 0), title="Divergence Turned Positive", message="Money Flow Divergence crossed above zero - Liquidity growing faster than comparison ticker")
alertcondition(ta.crossunder(money_flow_divergence, 0), title="Divergence Turned Negative", message="Money Flow Divergence crossed below zero - Comparison ticker growing faster than liquidity")
alertcondition(buy_signal, title="Entered Buy Zone", message="Money Flow Divergence entered BUY ZONE - Liquidity significantly outpacing comparison ticker")
