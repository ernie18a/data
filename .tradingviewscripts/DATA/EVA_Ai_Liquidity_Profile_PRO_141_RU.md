<!-- tradingview-pine-id: PUB;3724e236a8c941f287e7b775a964524e -->
<!-- tradingviewscripts-format: 1 -->
# 🧬 EVA Ai Liquidity Profile PRO 1.4.1 RU

Source: https://www.tradingview.com/script/nfZfeoCC-eva-ai-poc-liquidity-smart-money-1-4-1-ru/

## Description

💎 EVA Ai + POC, Liquidity & Smart Money 1.4.1 RU — РЫНОК, РАЗЛОЖЕННЫЙ ПО ЦЕНАМ
...
EVA Volume Profile PRO — аналитический инструмент для TradingView. Он не исполняет сделки и не является индивидуальной инвестиционной рекомендацией. Любой рынок связан с риском; окончательное решение, размер позиции и проверка сценария остаются за трейдером.

━━━━━━━━━━━━━━━━━━━━
🔥 ХОТИТЕ ПЕРЕЙТИ ОТ КАРТЫ ЛИКВИДНОСТИ К ГОТОВОМУ ТОРГОВОМУ СЦЕНАРИЮ?

EVA AI Plus объединяет подтверждённые LONG/SHORT-сигналы, Market Structure, Liquidity Sweep, Smart Money, Support/Resistance, Supply/Demand, Fair Value Gaps, Whale Volume, фильтры качества, AI Advisor, TP/SL, Trail, TP+ и внутреннюю статистику в одном рабочем пространстве TradingView.

Откройте премиальный EVA AI Plus, добавьте его в избранное и запросите доступ к 7-дневному тест-драйву:
https://ru.tradingview.com/script/gyJ8w0lV-eva-ai-plus-structure-and-liquidity-signals/
...
````pine
//@version=6
indicator("🧬 EVA Ai Liquidity Profile PRO 1.4.1 RU", shorttitle = "🧬 EVA Ai LP PREMIUM RU", overlay = true,
     max_bars_back = 5000, max_boxes_count = 500, max_lines_count = 500, max_labels_count = 500)

// =============================================================================
// EVA Liquidity Profile PRO 1.4.1 • Russian interface
...
const string G_RANGE  = "01 • Диапазон"
const string G_ENGINE = "02 • Профиль и точность"
const string G_LEVELS = "03 • POC и Value Area"
const string G_NODES  = "04 • Узлы профиля HVN / LVN"
const string G_POOLS  = "05 • Smart Liquidity Pools"
const string G_STYLE  = "06 • Neon Glow Style"
const string G_PANEL  = "07 • Инфопанель"

// ─────────────────────────────────────────────────────────────────────────────
// Inputs: autopilot
// ─────────────────────────────────────────────────────────────────────────────
bool autopilotInput = input.bool(true, "AUTOPILOT • автоадаптация", group = G_AUTO,
     tooltip = "Автоматически выбирает младший ТФ, плотность строк профиля, пороги HVN/LVN и параметры подтверждения ликвидности под текущий таймфрейм и объём данных.")
bool autopilotLiveProfileInput = input.bool(true, "Автообновление developing-профиля", group = G_AUTO,
     active = autopilotInput,
     tooltip = "Обновляет текущий профиль по поступающим подтверждённым микробарам. Smart Liquidity Pools всё равно создаются и удаляются только по закрытым свечам.")

// ─────────────────────────────────────────────────────────────────────────────
// Inputs: range
// ─────────────────────────────────────────────────────────────────────────────
string rangeModeInput = input.string(MODE_VISIBLE, "Режим профиля",
     options = [MODE_VISIBLE, MODE_SESSION, MODE_FIXED], group = G_RANGE,
     tooltip = "Visible Range — только видимые свечи. Session HD — отдельные профили последних торговых сессий. Fixed Range — диапазон между двумя перемещаемыми временными маркерами.",
     display = display.none)

int fixedStartInput = input.time(timestamp("01 Jan 2026 00:00 +0000"), "Fixed: начало",
     group = G_RANGE, active = rangeModeInput == MODE_FIXED,
     tooltip = "Маркер можно двигать мышью прямо на графике.", display = display.none)
int fixedEndInput = input.time(timestamp("31 Dec 2027 23:59 +0000"), "Fixed: конец",
     group = G_RANGE, active = rangeModeInput == MODE_FIXED,
     tooltip = "Маркер можно двигать мышью прямо на графике.", display = display.none)

string sessionHoursInput = input.session("0000-2359", "Session: торговые часы",
     group = G_RANGE, active = rangeModeInput == MODE_SESSION,
     tooltip = "Время биржи инструмента. Для полного торгового дня оставьте 0000–2359.",
     display = display.none)
int sessionProfilesInput = input.int(3, "Session: профилей на графике", minval = 1, maxval = 4,
     group = G_RANGE, active = rangeModeInput == MODE_SESSION, display = display.none)
int sessionLookbackBarsInput = input.int(3000, "Session: глубина расчёта, баров",
     minval = 500, maxval = 10000, step = 250, group = G_RANGE,
     active = rangeModeInput == MODE_SESSION,
     tooltip = "Ограничивает расчёт истории сессий и защищает Pine от перегрузки.",
     display = display.none)

bool confirmedOnlyInput = input.bool(true, "Только закрытые свечи",
     group = G_RANGE, active = not autopilotInput,
     tooltip = "Включено: профиль не меняется от тиков формирующейся свечи. Выключено: текущая свеча участвует в развивающемся профиле.")
...
string rowModeInput = input.string(ROWS_AUTO, "Row Size",
     ...
     display = display.none)
int rowsInput = input.int(64, "Auto Rows: максимум строк", minval = 12, maxval = 80,
     group = G_ENGINE, active = rowModeInput == ROWS_AUTO and not autopilotInput,
     display = display.none)
int ticksPerRowInput = input.int(10, "Ticks per Row: тиков в строке", minval = 1, maxval = 1000,
     group = G_ENGINE, active = rowModeInput == ROWS_TICKS and not autopilotInput,
     display = display.none)
int maxRowsInput = input.int(80, "Ticks per Row: максимум строк", minval = 12, maxval = 80,
     group = G_ENGINE, active = rowModeInput == ROWS_TICKS and not autopilotInput,
     tooltip = "Если выбранный шаг создаёт слишком много строк, профиль автоматически укрупняет шаг.",
     display = display.none)

string profileViewInput = input.string(VIEW_UPDOWN, "Вид гистограммы",
     options = [VIEW_UPDOWN, VIEW_TOTAL, VIEW_DELTA], group = G_ENGINE,
     tooltip = "Up / Down — нейтральный объёмный профиль с выровненной neon-рейкой доминирующей стороны у якоря профиля. Total Gradient — классический профиль без направленной рейки. Delta — ширина по абсолютному дисбалансу Up minus Down плюс направленная рейка. Направление рассчитывается по младшим свечам.",
     display = display.none)

bool useLtfInput = input.bool(true, "Повышенная точность по младшему ТФ",
     group = G_ENGINE, active = not autopilotInput,
     tooltip = "Распределяет объём по свечам младшего таймфрейма. При неполных данных автоматически используется полный набор свечей текущего графика.")
string lowerTfInput = input.timeframe("1", "Младший таймфрейм", group = G_ENGINE,
     active = useLtfInput and not autopilotInput, display = display.none)
int maxLtfSamplesInput = input.int(20000, "Лимит LTF-свечей", minval = 500, maxval = 60000,
     step = 500, group = G_ENGINE, active = useLtfInput or autopilotInput, display = display.none)
int maxProfileBarsInput = input.int(3000, "Лимит свечей диапазона", minval = 500, maxval = 10000,
     step = 250, group = G_ENGINE,
     tooltip = "При превышении лимита панель покажет предупреждение; скрыто обрезанный профиль не используется.",
     display = display.none)
...
bool showProfileInput = input.bool(true, "Показывать гистограмму", group = G_LEVELS)
bool showValueAreaFillInput = input.bool(false, "Подсвечивать Value Area", group = G_LEVELS,
     tooltip = "По умолчанию выключено: строки внутри VA уже выделены, поэтому график остаётся чистым.")
bool showLevelsInput = input.bool(true, "Линии POC / VAH / VAL", group = G_LEVELS)
bool extendLevelsInput = input.bool(true, "Продлевать уровни вправо", group = G_LEVELS,
     active = showLevelsInput)
bool showLevelLabelsInput = input.bool(true, "Подписи уровней", group = G_LEVELS,
     active = showLevelsInput)

// ─────────────────────────────────────────────────────────────────────────────
// Inputs: HVN / LVN liquidity zones
// ─────────────────────────────────────────────────────────────────────────────
bool showHvnInput = input.bool(true, "HVN — зоны высокой ликвидности", group = G_NODES)
int hvnCountInput = input.int(3, "HVN: максимум зон", minval = 1, maxval = 6,
     group = G_NODES, active = showHvnInput and not autopilotInput, display = display.none)
float hvnThresholdInput = input.float(55.0, "HVN: объём от POC, %", minval = 20.0,
     maxval = 95.0, step = 1.0, group = G_NODES, active = showHvnInput and not autopilotInput,
     display = display.none)

bool showLvnInput = input.bool(true, "LVN — зоны низкой ликвидности", group = G_NODES)
int lvnCountInput = input.int(3, "LVN: максимум зон", minval = 1, maxval = 6,
     group = G_NODES, active = showLvnInput and not autopilotInput, display = display.none)
float lvnThresholdInput = input.float(22.0, "LVN: объём не выше POC, %", minval = 1.0,
     maxval = 60.0, step = 1.0, group = G_NODES, active = showLvnInput and not autopilotInput,
     display = display.none)
float lvnProminenceInput = input.float(12.0, "LVN: минимальная глубина, %", minval = 0.0,
     maxval = 100.0, step = 1.0, group = G_NODES, active = showLvnInput and not autopilotInput,
     tooltip = "Насколько соседние строки должны быть объёмнее локальной впадины.",
     display = display.none)

int nodeSeparationInput = input.int(3, "Минимум между узлами, строк", minval = 1,
     maxval = 12, group = G_NODES, active = not autopilotInput, display = display.none)
bool adaptiveNodeZonesInput = input.bool(true, "Адаптивная ширина зон", group = G_NODES,
     tooltip = "Расширяет HVN/LVN по соседним строкам той же объёмной структуры.")
int nodeThicknessInput = input.int(1, "Фиксированная толщина, строк", minval = 1,
     maxval = 5, group = G_NODES, active = not adaptiveNodeZonesInput,
     display = display.none)
int nodeMaxThicknessInput = input.int(4, "Адаптивно: максимум строк", minval = 1,
     maxval = 7, group = G_NODES, active = adaptiveNodeZonesInput,
     display = display.none)
bool extendNodeZonesInput = input.bool(true, "Короткая проекция зон вправо", group = G_NODES,
     tooltip = "Проецирует HVN/LVN только на ближайший рабочий участок — 8% диапазона, но не более 30 баров. Бесконечная заливка отключена.")
bool showNodeLabelsInput = input.bool(true, "Подписи HVN / LVN", group = G_NODES)

// ─────────────────────────────────────────────────────────────────────────────
// Inputs: confirmed stop-liquidity pools
// ─────────────────────────────────────────────────────────────────────────────
bool showLiquidityPoolsInput = input.bool(true, "Smart Liquidity Pools", group = G_POOLS,
     tooltip = "BSL формируется у подтверждённых равных swing-high, SSL — у равных swing-low. Нужны минимум два касания и проход фильтра качества.")
int liquidityPivotInput = input.int(4, "Swing: плечо pivot", minval = 2, maxval = 12,
     group = G_POOLS, active = showLiquidityPoolsInput and not autopilotInput,
     display = display.none)
float liquidityAtrWidthInput = input.float(0.10, "Ширина зоны, ATR", minval = 0.03,
     maxval = 0.40, step = 0.01, group = G_POOLS,
     active = showLiquidityPoolsInput and not autopilotInput, display = display.none)
float liquidityQualityInput = input.float(72.0, "Минимальное качество, Q", minval = 50.0,
     maxval = 95.0, step = 1.0, group = G_POOLS,
     active = showLiquidityPoolsInput and not autopilotInput, display = display.none)
int liquidityLookbackInput = input.int(500, "Срок актуальности, баров", minval = 100,
     maxval = 2000, step = 50, group = G_POOLS, active = showLiquidityPoolsInput,
     display = display.none)
float liquidityVisibilityAtrInput = input.float(8.0, "Видимость зон, ATR", minval = 2.0,
     maxval = 30.0, step = 0.5, group = G_POOLS,
     active = showLiquidityPoolsInput and not autopilotInput, display = display.none,
     tooltip = "Далёкие зоны остаются в расчёте, но автоматически скрываются до возвращения цены в рабочий ATR-радиус.")
int maxLiquidityZonesInput = input.int(6, "Максимум активных зон", minval = 2,
     maxval = 10, group = G_POOLS, active = showLiquidityPoolsInput,
     display = display.none)
string liquiditySweepModeInput = input.string(SWEEP_FULL, "Когда зона собрана",
     options = [SWEEP_FULL, SWEEP_TOUCH], group = G_POOLS,
     active = showLiquidityPoolsInput, display = display.none,
     tooltip = "Full Sweep — удалить после прохождения дальней границы. First Touch — удалить сразу при первом входе цены в зону.")
bool showLiquidityLabelsInput = input.bool(true, "Подписи BSL / SSL и качество", group = G_POOLS,
     active = showLiquidityPoolsInput)

// ─────────────────────────────────────────────────────────────────────────────
// Inputs: style
// ─────────────────────────────────────────────────────────────────────────────
string profileSideInput = input.string(SIDE_RIGHT, "Сторона профиля",
     options = [SIDE_RIGHT, SIDE_LEFT], group = G_STYLE, display = display.none)
float profileWidthPctInput = input.float(27.0, "Ширина профиля, % диапазона",
     minval = 10.0, maxval = 70.0, step = 1.0, group = G_STYLE,
     display = display.none)
bool gradientOpacityInput = input.bool(true, "Объёмный градиент прозрачности", group = G_STYLE)
float rowGapPctInput = input.float(16.0, "Воздух между строками, %", minval = 0.0,
     maxval = 35.0, step = 1.0, group = G_STYLE, display = display.none,
     tooltip = "Небольшой зазор делает границы строк резкими и убирает эффект размазанной заливки.")

color outsideColorInput = input.color(color.rgb(98, 111, 176), "Объём вне VA", group = G_STYLE)
...
int outsideTransparencyInput = input.int(84, "Прозрачность вне VA", minval = 0,
     maxval = 100, group = G_STYLE, display = display.none)
int valueAreaTransparencyInput = input.int(62, "Прозрачность VA", minval = 0,
     maxval = 100, group = G_STYLE, display = display.none)
int zoneTransparencyInput = input.int(96, "Прозрачность зон", minval = 85,
     maxval = 99, group = G_STYLE, display = display.none)

bool showDashboardInput = input.bool(true, "Показывать инфопанель", group = G_PANEL)
...
f_liquidity_tooltip(bool isBuySide, float bottom, float top, float quality, int touches, bool tested) =>
    string sideText = isBuySide ? "BSL — buy-side liquidity над swing-high" : "SSL — sell-side liquidity под swing-low"
    string stateText = tested ? "TESTED — цена вошла в зону, ликвидность частично ослаблена" : "FRESH — подтверждённая зона ещё не тестировалась"
    string collectionText = liquiditySweepModeInput == SWEEP_FULL ? "удаление после полного sweep дальней границы" : "удаление при первом входе цены"
    sideText + "\nСостояние: " + stateText + "\nПодтверждений: " + str.tostring(touches) + "\nКачество: Q" + str.tostring(int(math.round(quality))) + "/100\nДиапазон: " + f_format_price(bottom) + " — " + f_format_price(top) + "\nЛогика качества: равные swing-уровни + относительный объём + rejection wick + дистанция.\nСбор ликвидности: " + collectionText + "."
            ...
            if totalVolume > 0.0 and maxVolume > 0.0
                ...
                if drawNodes and rowCount >= 3
                    ...
                    for nodeIndex in selectedHvn
                        ...
                        if isCurrent and showNodeLabelsInput
                            string hvnTooltip = "HVN — зона принятия цены\nДиапазон: " + f_format_price(zoneBottom) + " — " + f_format_price(zoneTop) + "\nПлотность: " + f_format_percent(nodePct) + " от POC\nОжидание: баланс, замедление или ретест."
                    ...
                    for nodeIndex in selectedLvn
                        ...
                        if isCurrent and showNodeLabelsInput
                            string lvnTooltip = "LVN — зона непринятия цены\nДиапазон: " + f_format_price(zoneBottom) + " — " + f_format_price(zoneTop) + "\nПлотность: " + f_format_percent(nodePct) + " от POC\nОжидание: быстрое прохождение либо резкий отказ."
                ...
                if showLevelsInput
                    ...
                    if isCurrent and showLevelLabelsInput
                        ...
                        string pocTooltip = "POC — максимум принятого объёма\nЦена: " + f_format_price(pocPrice) + "\nОбъём строки: " + f_format_volume(maxVolume) + " (" + f_format_percent(pocSharePct) + " профиля)\nДистанция цены: " + f_format_percent(pocDistancePct)
                        string vahTooltip = "VAH — верхняя граница Value Area\nЦена: " + f_format_price(vahPrice) + "\nФактическая VA: " + f_format_percent(actualVaPct) + "\nВыше VAH рынок переходит в поиск новой цены."
                        string valTooltip = "VAL — нижняя граница Value Area\nЦена: " + f_format_price(valPrice) + "\nФактическая VA: " + f_format_percent(actualVaPct) + "\nНиже VAL рынок переходит в поиск новой цены."
...
alertcondition(pocTouchEvent, "EVA • POC touch", "{{ticker}}: цена коснулась POC на {{close}}")
alertcondition(vahBreakEvent, "EVA • VAH breakout", "{{ticker}}: подтверждён выход выше VAH на {{close}}")
alertcondition(valBreakEvent, "EVA • VAL breakdown", "{{ticker}}: подтверждён выход ниже VAL на {{close}}")
alertcondition(returnToValueEvent, "EVA • Return to Value", "{{ticker}}: цена вернулась внутрь Value Area на {{close}}")
alertcondition(newBslEvent, "EVA • New BSL pool", "{{ticker}}: подтверждена качественная BSL-зона над swing-high")
alertcondition(newSslEvent, "EVA • New SSL pool", "{{ticker}}: подтверждена качественная SSL-зона под swing-low")
alertcondition(collectedBslEvent, "EVA • BSL collected", "{{ticker}}: buy-side liquidity собрана; BSL-зона удалена")
alertcondition(collectedSslEvent, "EVA • SSL collected", "{{ticker}}: sell-side liquidity собрана; SSL-зона удалена")
...
if barstate.islast
    if showDashboardInput
        ...
        string sourceText = dashboardUseLtf ? "LTF " + effectiveLowerTf : "Свечи графика"
        string rangeText = str.tostring(array.size(barOpenSamples)) + " бар • " + str.tostring(int(nz(currentSampleCount, 0))) + " сэмплов"
        string rowsText = na(currentRowCount) ? "—" : str.tostring(int(currentRowCount)) + " × " + f_format_price(currentRowStep)
        string statusText = barOverflow and not dashboardUseLtf ? "⚠ Диапазон превышен" : na(currentPoc) ? "Нет данных объёма" : effectiveUseLtf and (ltfOverflow or ltfIncomplete or not lowerTfIsValid) ? "Fallback без обрезки" : effectiveConfirmedOnly ? "Закрытые свечи" : "Developing / auto"
        ...
        string vaText = f_format_percent(currentActualVaPct) + " / цель " + f_format_percent(valueAreaPctInput)
        string nodesText = "H " + f_format_price(currentNearestHvn) + " • L " + f_format_price(currentNearestLvn)
        string structureText = na(currentPocPosition) ? "—" : currentPocPosition > 0.62 ? "Верхняя концентрация" : currentPocPosition < 0.38 ? "Нижняя концентрация" : "Сбалансированный"
        string auctionText = na(currentPoc) ? "НЕТ ПРОФИЛЯ" : close > currentVah ? "ВЫШЕ VAH • ПОИСК ЦЕНЫ ↑" : close < currentVal ? "НИЖЕ VAL • ПОИСК ЦЕНЫ ↓" : "ВНУТРИ VA • БАЛАНС"
        ...
        table.cell(dashboard, 0, 0, "EVA ЛИКВИДНОСТЬ • РАСЧЁТ",
             text_color = color.white, text_size = size.small,
             bgcolor = color.new(color.rgb(26, 35, 57), 0))
        table.cell(dashboard, 0, 1, "Автопилот", text_color = color.rgb(155, 169, 198), text_size = size.tiny, bgcolor = color.new(autopilotColor, 90))
        table.cell(dashboard, 1, 1, autopilotText, text_color = autopilotColor, text_size = size.tiny, bgcolor = color.new(autopilotColor, 90))
        table.cell(dashboard, 0, 2, "Аукцион", text_color = color.rgb(155, 169, 198), text_size = size.tiny, bgcolor = color.new(auctionColor, 88))
        table.cell(dashboard, 1, 2, auctionText, text_color = auctionColor, text_size = size.tiny, bgcolor = color.new(auctionColor, 88))
        table.cell(dashboard, 0, 3, "Режим", text_color = color.rgb(155, 169, 198), text_size = size.tiny)
        table.cell(dashboard, 1, 3, rangeModeInput + " • " + profileViewInput, text_color = color.white, text_size = size.tiny)
        table.cell(dashboard, 0, 4, "Источник", text_color = color.rgb(155, 169, 198), text_size = size.tiny)
        table.cell(dashboard, 1, 4, sourceText, text_color = dashboardUseLtf ? color.rgb(79, 224, 177) : color.white, text_size = size.tiny)
        table.cell(dashboard, 0, 5, "Диапазон", text_color = color.rgb(155, 169, 198), text_size = size.tiny)
        table.cell(dashboard, 1, 5, rangeText, text_color = color.white, text_size = size.tiny)
        table.cell(dashboard, 0, 6, "Строки × шаг", text_color = color.rgb(155, 169, 198), text_size = size.tiny)
        ...
        table.cell(dashboard, 0, 8, "Дельта", text_color = color.rgb(155, 169, 198), text_size = size.tiny)
        table.cell(dashboard, 1, 8, deltaText, text_color = deltaColor, text_size = size.tiny)
        table.cell(dashboard, 0, 9, "POC / откл.", text_color = pocColorInput, text_size = size.tiny)
        ...
        table.cell(dashboard, 0, 11, "Зона стоимости", text_color = valueAreaColorInput, text_size = size.tiny)
        table.cell(dashboard, 1, 11, vaText, text_color = valueAreaColorInput, text_size = size.tiny)
        table.cell(dashboard, 0, 12, "Ближ. BSL", text_color = buyLiquidityColorInput, text_size = size.tiny)
        table.cell(dashboard, 1, 12, bslText, text_color = bslPanelColor, text_size = size.tiny)
        table.cell(dashboard, 0, 13, "Ближ. SSL", text_color = sellLiquidityColorInput, text_size = size.tiny)
        table.cell(dashboard, 1, 13, sslText, text_color = sslPanelColor, text_size = size.tiny)
        table.cell(dashboard, 0, 14, "Ближ. HVN/LVN", text_color = color.rgb(155, 169, 198), text_size = size.tiny)
        table.cell(dashboard, 1, 14, nodesText, text_color = color.white, text_size = size.tiny)
        table.cell(dashboard, 0, 15, "Структура", text_color = color.rgb(155, 169, 198), text_size = size.tiny)
        table.cell(dashboard, 1, 15, structureText, text_color = color.white, text_size = size.tiny)
        table.cell(dashboard, 0, 16, "Статус", text_color = color.rgb(155, 169, 198), text_size = size.tiny)
