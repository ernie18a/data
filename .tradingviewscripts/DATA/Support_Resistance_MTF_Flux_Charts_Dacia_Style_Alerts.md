<!-- tradingview-pine-id: PUB;96b9c651d05842e1b665c9215960ca96 -->
<!-- tradingviewscripts-format: 1 -->
# Support & Resistance (MTF) | Flux Charts + Dacia Style & Alerts

Source: https://www.tradingview.com/script/Y4qpBPNl-Support-Resistance-MTF-Dacia-Style-Alerts/
...
indicator("Support & Resistance (MTF) | Flux Charts + Dacia Style & Alerts", overlay = true, max_labels_count = 500, max_lines_count = 500, max_boxes_count = 500, dynamic_requests = true)
...
styleMode = input.string("Lines", "Style", ["Lines", "Zones"], group = "Style")
lineStyle = input.string("____", "Line Style", ["____", "----", "...."], group = "Style")
lineWidth = input.int(2, "Line Width / Boldness", minval = 1, maxval = 5, group = "Style")
zoneSize = input.float(1.0, "Zone Width", minval = 0.1, maxval = 10, step = 0.1, group = "Style")
zoneSizeATR = zoneSize * 0.075
zoneTransparency = input.int(75, "Zone Fill Transparency", minval = 0, maxval = 100, group = "Style")
zoneBorderWidth = input.int(2, "Zone Border Width / Boldness", minval = 0, maxval = 5, group = "Style")
supportColor = input.color(#089981, "Support Fill / Line", group = "Style", inline = "RScolors")
resistanceColor = input.color(#f23645, "Resistance Fill / Line", group = "Style", inline = "RScolors")
supportBorderColor = input.color(#089981, "Support Border", group = "Style", inline = "bordercolors")
resistanceBorderColor = input.color(#f23645, "Resistance Border", group = "Style", inline = "bordercolors")
breakColor = input.color(color.blue, "Break Marker", group = "Style", inline = "RScolors2")
textColor = input.color(#ffffff, "Text Color", group = "Style", inline = "RScolors2")
labelSizeInput = input.string("Small", "Line / Zone Label Font", options = ["Tiny", "Small", "Normal", "Large"], group = "Style")
eventLabelSizeInput = input.string("Small", "B / R Marker Font", options = ["Tiny", "Small", "Normal", "Large"], group = "Style")

allAlertsMaster = input.bool(true, "All Alerts Master Switch", group = "Alerts")
enableRetestAlerts = input.bool(true, "Enable Retest Alerts", tooltip = "Needs Show Retests enabled.", group = "Alerts")
enableBreakAlerts = input.bool(true, "Enable Break Alerts", tooltip = "Needs Show Breaks enabled.", group = "Alerts")
enableAllRetestAlert = input.bool(true, "ALL S/R Retests", group = "Alerts")
enableAllBreakAlert = input.bool(true, "ALL S/R Breaks", group = "Alerts")
enableAllEventsAlert = input.bool(true, "ALL S/R Events", group = "Alerts")
...
textSizeFromInput (sizeInput) =>
    sizeInput == "Tiny" ? size.tiny : sizeInput == "Normal" ? size.normal : sizeInput == "Large" ? size.large : size.small
...
renderSRObj (srObj sr) =>
    if na(sr.info.breakTime) or showInvalidated
        ...
        if styleMode == "Lines"
            ...
            // Label
            sr.srLabel := label.new(extendType == extend.none ? ((sr.info.startTime + endTime) / 2) : endTime, sr.info.price, xloc = xloc.bar_time, text = labelTitle, textcolor = textColor, style = label.style_none, size = textSizeFromInput(labelSizeInput))
        else
            // Zone
            zoneFillColor = sr.info.srType == "Resistance" ? color.new(resistanceColor, zoneTransparency) : color.new(supportColor, zoneTransparency)
            zoneBorderColor = sr.info.srType == "Resistance" ? resistanceBorderColor : supportBorderColor
            sr.srBox := box.new(sr.info.startTime, sr.info.price + atr * zoneSizeATR, endTime, sr.info.price - atr * zoneSizeATR, xloc = xloc.bar_time, bgcolor = zoneFillColor, border_color = zoneBorderColor, border_width = zoneBorderWidth, text = labelTitle, text_color = textColor, extend = extendType, text_size = textSizeFromInput(labelSizeInput), text_halign = (extendType != extend.none) ? text.align_right : text.align_center)

        // Break Label
        if showBreaks
            if not na(sr.info.breakTime)
                sr.breakLabel := label.new(sr.info.breakTime, sr.info.price, "B", yloc = sr.info.srType ==  "Resistance" ? yloc.belowbar : yloc.abovebar, style = sr.info.srType == "Resistance" ? label.style_label_up : label.style_label_down, color = breakColor, textcolor = color.new(textColor, 0), xloc = xloc.bar_time, size = textSizeFromInput(eventLabelSizeInput))
...
bool srRetestEvent = alerts.get("Retest")
bool srBreakEvent = alerts.get("Break")
bool srAnyEvent = srRetestEvent or srBreakEvent

alertcondition(allAlertsMaster and enableRetestAlerts and srRetestEvent, "New Retest", "New S/R retest.")
alertcondition(allAlertsMaster and enableBreakAlerts and srBreakEvent, "New Break", "New S/R break.")

alertcondition(allAlertsMaster and enableAllRetestAlert and srRetestEvent, "ALL Support / Resistance Retests", "{{ticker}}: New support or resistance retest.")
alertcondition(allAlertsMaster and enableAllBreakAlert and srBreakEvent, "ALL Support / Resistance Breaks", "{{ticker}}: New support or resistance break.")
alertcondition(allAlertsMaster and enableAllEventsAlert and srAnyEvent, "ALL Support / Resistance Events", "{{ticker}}: New support/resistance break or retest.")

if allAlertsMaster and enableAllEventsAlert and srAnyEvent
    string srMessage = syminfo.ticker + " | " + timeframe.period + " | "
    if srRetestEvent and enableRetestAlerts
        srMessage += "S/R retest; "
    if srBreakEvent and enableBreakAlerts
        srMessage += "S/R break; "
    alert(srMessage, alert.freq_once_per_bar_close)
````
