<!-- tradingview-pine-id: PUB;8146237255f6411b9cdfa9b3c701a320 -->
<!-- tradingviewscripts-format: 1 -->
# Market Open AMD + Liquidity Sweep IFVG

Source: https://www.tradingview.com/script/53QOVo7V-Market-Open-AMD-BBT-MODEL/

## Description

Marks out accumulation, manipulation, distribution zones. Presents FVG and IFVG. Marks out overnight liquidity. Colors fully customizable. Marks out highs and lows of A&M.
...
accSession = input.session(
     "0830-0930",
     "Accumulation Time",
     group="AMD Sessions"
)

manSession = input.session(
     "0930-1000",
     "Manipulation Time",
     group="AMD Sessions"
)

distSession = input.session(
     "1000-1600",
     "Distribution Time",
     group="AMD Sessions"
)

showAMDBoxes = input.bool(
     true,
     "Show AMD Boxes",
     group="AMD Sessions"
)
...
liquidityStartMinutes =
     liquidityStartHour * 60 + liquidityStartMinute

liquidityEndMinutes =
     liquidityEndHour * 60 + liquidityEndMinute

accLineStartMinutes =
     accLineStartHour * 60 + accLineStartMinute

accLineEndMinutes =
     accLineEndHour * 60 + accLineEndMinute

manLineStartMinutes =
     manLineStartHour * 60 + manLineStartMinute

manLineEndMinutes =
     manLineEndHour * 60 + manLineEndMinute
...
// The lines only extend during the selected liquidity range.
// They stop permanently at the configured end time.
...
if showAMDBoxes and inManipulation
    ...
    if not inManipulation[1]
        ...
        box.set_top(manBox, math.max(box.get_top(manBox), high))
        box.set_bottom(manBox, math.min(box.get_bottom(manBox), low))
...
if showAMDBoxes and inDistribution
    ...
    if not inDistribution[1]
        ...
        box.set_top(distBox, math.max(box.get_top(distBox), high))
        box.set_bottom(distBox, math.min(box.get_bottom(distBox), low))
...
if showAMDLines and not na(lockedAccHigh)
    ...
    if currentMinutes >= accLineStartMinutes and na(accHighLine)
    ...
    if not na(accHighLine) and currentMinutes < accLineEndMinutes
...
if showAMDLines and not na(lockedAccLow)
    ...
    if currentMinutes >= accLineStartMinutes and na(accLowLine)
    ...
    if not na(accLowLine) and currentMinutes < accLineEndMinutes
...
if showAMDLines and not na(manHigh)
    ...
    if currentMinutes >= manLineStartMinutes and na(manHighLine)
    ...
    if not na(manHighLine) and currentMinutes < manLineEndMinutes
...
if showAMDLines and not na(manLow)
    ...
    if currentMinutes >= manLineStartMinutes and na(manLowLine)
    ...
    if not na(manLowLine) and currentMinutes < manLineEndMinutes
...
// Sweep ABOVE liquidity = potential bearish setup
bearishSweep = (
     signalPhaseActive and
...
// Sweep BELOW liquidity = potential bullish setup
bullishSweep = (
     signalPhaseActive and
...
// A bearish FVG becomes bullish IFVG when price actually crosses above it.
//
// The close[1] condition makes this an actual crossing event rather than
// repeatedly detecting the same FVG on multiple candles.
...
if bullishIFVGFormed
    ...
    // DELETE THE ORIGINAL FVG
    // This prevents the FVG color from showing underneath the IFVG.
    if not na(bearFVGBox)

        box.delete(bearFVGBox)
        bearFVGBox := na

    // CREATE EXACTLY ONE SOLID BULLISH IFVG
    box.new(
         left=bearFVGBar - 2,
         top=bearFVGTop,
    ...
    // Clear the FVG so it cannot create another IFVG
    bearFVGActive := false
    bearFVGTop := na
    bearFVGBottom := na
    bearFVGBar := na
...
// A bullish FVG becomes bearish IFVG when price crosses below it.
...
if bearishIFVGFormed
    ...
    // CREATE EXACTLY ONE SOLID BEARISH IFVG
    box.new(
         left=bullFVGBar - 2,
         top=bullFVGTop,
         right=bar_index + ifvgExtensionBars,
...
// ============================================================================
// CLEAN UP OLD FVG REFERENCES
// ============================================================================

// If an FVG has passed its IFVG validity window without becoming an IFVG,
// stop treating it as an active IFVG candidate.
