<!-- tradingview-pine-id: PUB;e4357ac088c343b38fc832687d207c78 -->
<!-- tradingviewscripts-format: 1 -->
# Macro Event Radar JP Free

Source: https://www.tradingview.com/script/DHkSpSfp/

## Description

FX向けの日本語経済指標カレンダーです。

公開Pine Seedsフィードから予定を自動取得し、日時を日本時間（JST）で表示します。通貨・重要度・指標名・発表までの残り時間・状態をチャート上で確認できます。

【主な機能】
・経済指標予定の自動取得
・JST日時、通貨、重要度、残り時間の一覧表示
・重要度「高・中・低」の個別表示フィルター
・対象通貨を「すべて・チャートに自動連動・手動指定」から選択
・y/y、m/m、q/q、w/wを「前年比・前月比・前期比・前週比」へ日本語化
・CPI、PPI、GDP、FOMCなど主要指標の日本語補助表記
・発表時刻の縦線、発表済みラベル、警戒時間の背景表示
・発表前の動的アラート
・発表後5分、15分、30分、60分の値動きとMFE／MAE分析
・自動取得が利用できない場合の手動JST予定入力

【使い方】
1. 「チャートで使用する」からインジケーターを追加します。
2. 「予定データ取得」は通常「自動」のまま使用します。
3. 設定の「表示フィルター」で、高・中・低から表示したい重要度を選びます。
4. 「対象通貨＝自動」にすると、現在のチャートに関連する通貨へ絞り込めます。
5. アラート作成画面で本インジケーターを選び、「Any alert() function call」を指定すると事前通知を利用できます。
6. 予定表は右上、反応分析は左下が初期配置です。設定から位置を変更できます。

【データと免責事項】
経済指標データには遅延・欠落・予定変更の可能性があります。重要な発表時刻は各国の政府機関・中央銀行などの公式情報でも確認してください。本インジケーターは投資助言ではありません。

【クレジット】
データフィード・公開ライブラリ：toodegrees
参照データ：Forex Factory
日本語UI・JST表示・重要度フィルター・反応分析：a4gete02b

English summary:
A free Japanese economic-calendar overlay with automatic public-feed import, JST display, per-impact filters, alerts and post-release reaction analysis. Data may be delayed or incomplete; verify important events with official sources.
