<!-- tradingview-pine-id: PUB;4e72f382c001417ba5554bb4053c77a8 -->
<!-- tradingviewscripts-format: 1 -->
# Dacia Smoothed Heikin Ashi + Multi EMA Suite V1.2 Visibility Toggle

Source: https://www.tradingview.com/script/cKxaY4DA-Dacia-Smoothed-Heikin-Ashi-EMA-9-21-50-200-Alerts/

## Description

Dacia Smoothed Heikin Ashi + EMA 9/21/50/200 Alerts
...
````pine
//@version=6
indicator("Dacia Smoothed Heikin Ashi + Multi EMA Suite V1.2 Visibility Toggle", shorttitle="Dacia SHA + EMA V1.2", overlay=true, max_labels_count=100)

// 1. Smoothed Heikin Ashi
string shaGroup = "1. Smoothed Heikin Ashi"
int smoothLength1 = input.int(10, "First Smoothing Length", minval=1, maxval=200, group=shaGroup)
int smoothLength2 = input.int(10, "Second Smoothing Length", minval=1, maxval=200, group=shaGroup)
bool showSHACandles = input.bool(true, "Show Smoothed HA Candles", group=shaGroup)

bool useTransparentCandles = input.bool(
     true,
     "Use Transparent SHA Candles",
     tooltip="Turn this off for fully solid smoothed Heikin Ashi candles.",
     group=shaGroup)
...
int bodyTransparency = input.int(
     ...
     minval=0,
     maxval=100,
     tooltip="Higher values make the smoothed candles more transparent so EMA crossings remain visible.",
     group=shaGroup)
...
int borderTransparency = input.int(
     ...
     maxval=100,
     group=shaGroup)
color bullBodyColor = input.color(color.lime, "Bullish Body", inline="body", group=shaGroup)
color bearBodyColor = input.color(color.red, "Bearish Body", inline="body", group=shaGroup)
color bullWickColor = input.color(color.lime, "Bullish Wick", inline="wick", group=shaGroup)
color bearWickColor = input.color(color.red, "Bearish Wick", inline="wick", group=shaGroup)
color bullBorderColor = input.color(color.lime, "Bullish Border", inline="border", group=shaGroup)
color bearBorderColor = input.color(color.red, "Bearish Border", inline="border", group=shaGroup)

float o = ta.ema(open, smoothLength1)
float h = ta.ema(high, smoothLength1)
float l = ta.ema(low, smoothLength1)
float c = ta.ema(close, smoothLength1)
float haClose = (o + h + l + c) / 4.0
var float haOpen = na
haOpen := na(haOpen[1]) ? (o + c) / 2.0 : (haOpen[1] + haClose[1]) / 2.0
float haHigh = math.max(h, math.max(haOpen, haClose))
float haLow = math.min(l, math.min(haOpen, haClose))
float shaOpen = ta.ema(haOpen, smoothLength2)
float shaHigh = ta.ema(haHigh, smoothLength2)
float shaLow = ta.ema(haLow, smoothLength2)
float shaClose = ta.ema(haClose, smoothLength2)
bool shaBull = shaClose >= shaOpen
color rawBodyColor = shaBull ? bullBodyColor : bearBodyColor
color rawWickColor = shaBull ? bullWickColor : bearWickColor
color rawBorderColor = shaBull ? bullBorderColor : bearBorderColor
...
plotcandle(showSHACandles ? shaOpen : na, showSHACandles ? shaHigh : na, showSHACandles ? shaLow : na, showSHACandles ? shaClose : na, title="Smoothed Heikin Ashi", color=bodyColor, wickcolor=wickColor, bordercolor=borderColor)

// 2. EMA source
string emaGroup = "2. EMA Lines"
string emaSourceInput = input.string("Smoothed HA Close", "EMA Source", options=["Smoothed HA Close", "Regular Close"], group=emaGroup)
float emaSource = emaSourceInput == "Smoothed HA Close" ? shaClose : close
...
string fastChoice = input.string("5", "Fast EMA", options=["5", "8", "9", "10", "13", "20", "21", "50", "100", "200", "Custom 1", "Custom 2", "Custom 3"], group=crossGroup)
string slowChoice = input.string("13", "Slow EMA", options=["5", "8", "9", "10", "13", "20", "21", "50", "100", "200", "Custom 1", "Custom 2", "Custom 3"], group=crossGroup)
...
string signalGroup = "7. SHA Position Signals"
bool showPositionMarkers = input.bool(false, "Show SHA Position Markers", group=signalGroup)
bool requireBodyAboveBelow = input.bool(true, "Require Entire SHA Body Above / Below Pair", group=signalGroup)
float bodyTop = math.max(shaOpen, shaClose)
float bodyBottom = math.min(shaOpen, shaClose)
bool bullishPosition = shaBull and selectedFast > selectedSlow and (requireBodyAboveBelow ? bodyBottom > selectedFast and bodyBottom > selectedSlow : shaClose > selectedFast and shaClose > selectedSlow)
bool bearishPosition = not shaBull and selectedFast < selectedSlow and (requireBodyAboveBelow ? bodyTop < selectedFast and bodyTop < selectedSlow : shaClose < selectedFast and shaClose < selectedSlow)
...
plotshape(showPositionMarkers and newBullishPosition, title="Bullish SHA Above EMA Pair", style=shape.labelup, location=location.belowbar, color=color.lime, textcolor=color.black, size=size.tiny, text="SHA+")
plotshape(showPositionMarkers and newBearishPosition, title="Bearish SHA Below EMA Pair", style=shape.labeldown, location=location.abovebar, color=color.red, textcolor=color.white, size=size.tiny, text="SHA-")
...
bool alertSHAPosition = input.bool(false, "Alert On New SHA Above / Below Pair", tooltip="Optional confirmation alert. EMA cross alerts are the cleaner default.", group=alertGroup)
...
alertcondition(bullishPositionAlert, "Bullish SHA Above EMA Pair", "{{ticker}} {{interval}}: Bullish smoothed Heikin Ashi body moved above the selected EMA pair.")
alertcondition(bearishPositionAlert, "Bearish SHA Below EMA Pair", "{{ticker}} {{interval}}: Bearish smoothed Heikin Ashi body moved below the selected EMA pair.")
alertcondition(bullishCrossAlert or bearishCrossAlert or bullishPositionAlert or bearishPositionAlert, "ALL SHA + EMA Events", "{{ticker}} {{interval}}: Smoothed Heikin Ashi and EMA event.")
...
if bullishCrossAlert or bearishCrossAlert or bullishPositionAlert or bearishPositionAlert
    ...
    if bullishPositionAlert
        message += "Bullish SHA above EMA pair; "
    if bearishPositionAlert
        message += "Bearish SHA below EMA pair; "
