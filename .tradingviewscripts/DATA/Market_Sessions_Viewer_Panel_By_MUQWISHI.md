...
Source: https://www.tradingview.com/script/htAF8NF9-Market-Sessions-Viewer-Panel-By-MUQWISHI/
...
▋ OVERVIEW:
[image]https://www.tradingview.com/x/z3RJkCvh/[/image]

_______________________
▋ CREDIT:
This indicator utilizes the “[Timezone — Library](https://www.tradingview.com/script/pvlt0Jwm-Timezone/)”. A huge thanks to @n00btraders for effort and well-organized work.

_______________________
▋ SESSION PANEL:
The Session Panel allows traders to visually compare session volatility using a candlestick/bar pattern. 

[image]https://www.tradingview.com/x/hT9Kqes4/[/image]
...
Chart Type:
[image]https://www.tradingview.com/x/VunLJQ26/[/image]
It offers two styles Bar or Candle to display based on traders’ preference

Sorting: 
[image]https://www.tradingview.com/x/hAFhgQi8/[/image]
...
_______________________
▋ CHART VISUALIZATION:
The chart visualization highlights each trading session using color-coded backgrounds in two selectable drawing styles [Candle or Box] that span their respective active timeframes. Each session block provides session’s name, close price, and change from open.

Chart Type: Candle
[image]https://www.tradingview.com/x/QUNPAEaE/[/image]

Chart Type: Box
[image]https://www.tradingview.com/x/7QQTJEJA/[/image]

Extra Drawing Feature:
[image]https://www.tradingview.com/x/Il19C5ur/[/image]
...
▋ INDICATOR SETTINGS:
Section(1): Sessions
[image]https://www.tradingview.com/x/EziDhJ89/[/image]
(1) Universal Timezone.
(2) Each Session: Enable/Disable, Name, Color, and Time. 

Section(2): Session Panel
[image]https://www.tradingview.com/x/0e7gKBhF/[/image]
...
[image]https://www.tradingview.com/x/bLKljgsF/[/image]
...
var G1 = "Sessions"
// Session No1
SESS01 = input.bool(true, "Session №1",                group = G1, display = display.none, inline = "1")
sess01 = sessions.new(SESS01,
     input.string("Tokyo", "",                         group = G1, display = display.none, inline = "1", active = SESS01),
     input.color(color.new(cl.DarkMagenta, 80), "",    group = G1, display = display.none, inline = "1", active = SESS01),
     input.session("0000-0900", sp,                    group = G1, display = display.none,               active = SESS01))

// Session No2
SESS02 = input.bool(true, "Session №2",                group = G1, display = display.none, inline = "2")
sess02 = sessions.new(SESS02, 
     input.string("London", "",                        group = G1, display = display.none, inline = "2", active = SESS02),
     input.color(color.new(cl.OrangeRed, 80), "",      group = G1, display = display.none, inline = "2", active = SESS02),
     input.session("0700-1500", "",                    group = G1, display = display.none,               active = SESS02))

// Session No3
SESS03 = input.bool(true, "Session №3",                group = G1, display = display.none, inline = "3")
sess03 = sessions.new(SESS03,
     input.string("New York", "",                      group = G1, display = display.none, inline = "3", active = SESS03),
     input.color(color.new(cl.DarkTurquoise , 80), "", group = G1, display = display.none, inline = "3", active = SESS03),
     input.session("1300-2000", "",                    group = G1, display = display.none,               active = SESS03))

// Session No4
SESS04 = input.bool(false, "Session №4",                group = G1, display = display.none, inline = "4"),
sess04 = sessions.new(SESS04,
     input.string("Sydney", "",                         group = G1, display = display.none, inline = "4", active = SESS04),
     input.color(color.new(cl.LightSlateGray , 80), "", group = G1, display = display.none, inline = "4", active = SESS04),
     input.session("2100-0600", "",                     group = G1, display = display.none,               active = SESS04))
...
snpTyp = input.string("Candle", "            Chart Types", ["Candle", "Bar"],  
                                                     group = G2, display = display.none, inline = "2", active = snpChk)
snpCl1 = input.color(cl.Green, "   ",                group = G2, display = display.none, inline = "2", active = snpChk)
snpCl2 = input.color(cl.Red, "  ",                   group = G2, display = display.none, inline = "2", active = snpChk)

snpWd  = input.float(1.5, "            W x H             ", 0.01, step = 0.1,
                                                     group = G2, display = display.none, inline = "3", active = snpChk)
snpHi  = input.float(2 , "x", 0.01, step = 0.1,      group = G2, display = display.none, inline = "3", active = snpChk)/10

snpLoc = input.string(  "Bottom Right"  , "            Location       ", 
          ["Top Right" , "Middle Right" , "Bottom Right"   , 
           "Top Center", "Middle Center", "Bottom Center"  , 
           "Top Left"  , "Middle Left"  , "Bottom Left" ]  , group = G2, display = display.none, inline = "4", active = snpChk)

snpSrt = input.string( "Right to Left"  , "            Sort                 ", 
          ["Default", "Right to Left" , "Left to Right"],    group = G2, display = display.none, inline = "5", active = snpChk)
...
drwTyp = input.string("Box", "            Draw Shape", ["Box", "Candle"],  
                                                             group = G3, display = display.none, inline = "2", active = drwChk)

drwSyl = input.string("-----------", "            Style | Size ",  ["________", "-----------", "..........."],      
                                                             group = G3, display = display.none, inline = "3", active = drwChk)
drwSiz = input.int(1, "", 0,                                 group = G3, display = display.none, inline = "3", active = drwChk)

labLoc = input.string(  "Above"     , "            Label Point  ", 
                                         ["Above", "Below"], group = G3, display = display.none, inline = "4", active = drwChk)
labSiz = input.int(12, "", 0,                                group = G3, display = display.none, inline = "4", active = drwChk)

nmeLab = input.bool(true, "Show Name             ",          group = G3, display = display.none, inline = "6", active = drwChk)
prcLab = input.bool(true, "Show Last",                       group = G3, display = display.none, inline = "6", active = drwChk)
chgLab = input.bool(true, "Show Change         ",            group = G3, display = display.none, inline = "7", active = drwChk)
chpLab = input.bool(true, "Show Change(%)",                  group = G3, display = display.none, inline = "7", active = drwChk)
...
// +++++++++++++++ Current Bar Time in HHmm format
barTime = str.tonumber(str.format_time(time, "HHmm", timZon))
...
newLab(x, y, txtClr, txt) =>
    ...
    label.new(x, y, txt, xloc.bar_index, yloc.price, cl.cNone, style, txtClr, labSiz)
...
method newDraw(chartVisual this, data prc, xLimit) =>
    ...
    if drwChk
        ...
        // Highlight
        this.highlight := newBox(prc.hi, prc.sBr + xLimit, prc.lw, cl.cNone, color1)
...
// +++++++++++++++ Main
itsTime      = time(timeframe.period, timZon)
dayWeek      = dayofweek(itsTime)
var tradeDay = 0, tradeDay := dayWeek != dayWeek[1] ? tradeDay + 1 : tradeDay
...
method main(sessions this, int key) =>
    ...
    if timeframe.isintraday
        ...
        if enable and key < 5       
            ...
            // Define Variable To Record Number of bars per sesseion per day
            var day = dayWeek - 1
            ...
            timeCond = (barTime >= sSessTime and barTime[1] <= eSessTime and barTime < barTime[1])
            ...
            if (not na(session) and na(session[1])) or timeCond
                ...
                prc := data.new("🟢", tradeDay, bar_index, bar_index, open, close, high, low, volume, this)
...
point(x) => 
    ...
    40 - math.round(40/(maxHi - minLw) * (x - minLw)) + 2
...
if barstate.islast
    ...
    if snpChk and mxSz > 0
        plt := table.new(locNsze(snpLoc), nCol, nRow, cl.cNone, chart.fg_color, 1, cl.cNone, 0)
        ...
        for i  = (snpSrt == "Left to Right" ? values.size() - 1 : 0) to 
            ...
            // Info Market Session 
            infoCell(k, 0, status + "\n" + name + "\n" +  toStr(cs) + "\n(" 
                         + str.tostring((cs - op)/op * 100, format.percent) + ")", color2, color1, statusTip)
