...
Source: https://www.tradingview.com/script/MmfInH17-Zigzag/
...
// @field subPivots direct sub pivots of multi level zigzag
// @field micropivots all micropivots of the the multi level zigzag
...
method tostring(chart.point this) =>
    str.tostring(this.price)
method get(Pivot this, string key) =>
    key == 'point' ? this.point.tostring() : key == 'dir' ? str.tostring(this.dir) : key == 'level' ? str.tostring(this.level) : key == 'componentIndex' ? str.tostring(this.componentIndex) : key == 'subComponents' ? str.tostring(this.subComponents) : key == 'microComponents' ? str.tostring(this.microComponents) : key == 'ratio' ? str.tostring(this.ratio) : key == 'sizeRatio' ? str.tostring(this.sizeRatio) : key == 'barRatio' ? str.tostring(this.barRatio) : key == 'indicatorNames' ? na(this.indicatorNames) ? '[]' : str.tostring(this.indicatorNames) : key == 'indicatorValues' ? na(this.indicatorValues) ? '[]' : str.tostring(this.indicatorValues) : key == 'indicatorRatios' ? na(this.indicatorRatios) ? '[]' : str.tostring(this.indicatorRatios) : na
...
// @returns string representation of ZigzagFlags
export method tostring(ZigzagFlags this) =>
    str.tostring(array.from(this.newPivot, this.doublePivot, this.updateLastPivot))
...
method addnewpivot(Zigzag this, Pivot pivot, array<Pivot> components = na) =>
    ...
    if this.zigzagPivots.size() >= 1
        ...
        if not na(components) ? components.size() > lastPivot.componentIndex : false
            ...
            pivot.microComponents
            // if(pivot.micropivots.getBars().last() == pivot.micropivots.getBars().get(pivot.micropivots.getBars().size()-2))
            //     log.info('Level : {0}', pivot.level)
            //     log.info('Micropivots : {0}', pivot.micropivots.getBars())
            //     log.info('Subpivots : {0}', pivot.subPivots.getBars())
...
method get(Zigzag this, string key) =>
    key == 'length' ? str.tostring(this.length) : key == 'numberOfPivots' ? str.tostring(this.numberOfPivots) : key == 'offset' ? str.tostring(this.offset) : key == 'level' ? str.tostring(this.level) : key == 'zigzagPivots' ? na(this.zigzagPivots) ? '[]' : this.zigzagPivots.tostring() : key == 'flags' ? na(this.flags) ? '[]' : this.flags.tostring() : na
...
export method nextlevel(Zigzag this) =>
    ...
    if this.zigzagPivots.size() > 0
        ...
        for i = this.zigzagPivots.size() - 1 to 0 by 1
            ...
            lPivot.micropivots := array.new<Pivot>()
            dir = lPivot.dir
...
method draw_zg_line(ZigzagDrawing this, idx1) =>
    ...
    if this.zigzag.zigzagPivots.size() > idx2
        ...
        zline = this.properties.force_overlay ? 
                 line.new(lastPivot.point, llastPivot.point, xloc = this.properties.xloc, color = this.properties.lineColor, width = this.properties.lineWidth, style = this.properties.lineStyle, force_overlay = true) :
                 line.new(lastPivot.point, llastPivot.point, xloc = this.properties.xloc, color = this.properties.lineColor, width = this.properties.lineWidth, style = this.properties.lineStyle)
        ...
        if this.properties.showLabel
            ...
            zlabel := this.properties.force_overlay ? 
                         label.new(lastPivot.point, yloc = yloc.price, xloc = this.properties.xloc, color = labelColor, style = labelStyle, text = hhllText, textcolor = this.properties.textColor, size = size.small, force_overlay = true):
                         label.new(lastPivot.point, yloc = yloc.price, xloc = this.properties.xloc, color = labelColor, style = labelStyle, text = hhllText, textcolor = this.properties.textColor, size = size.small)
            zlabel
...
export method drawplain(ZigzagDrawing this) =>
    ...
    if barstate.islast or barstate.islastconfirmedhistory
        for i = 0 to this.zigzag.zigzagPivots.size() > 1 ? this.zigzag.zigzagPivots.size() - 2 : na by 1
            ...
            if this.properties.showLabel
                ...
                labelText = (this.properties.showLevels ? '(Level ' + str.tostring(level) + ' : ' + str.tostring(lastPivot.ratio) + '/' + str.tostring(lastPivot.barRatio) + ')' : '') +
                                 (this.properties.showSubcomponents ? (lastPivot.subComponents != 0 ? '\nSC :' + 
                                                 str.tostring(lastPivot.subComponents) + '/' : level > 0 ? '\nMC : ' : '') + 
                                                 (level > 0 ? str.tostring(lastPivot.microComponents) : '') + (not na(lastPivot.subPivots) ? ' (' + 
                                                 subPivotValues.join(',') + ') ' : '') : '')
                ...
                zlabel := this.properties.force_overlay? 
                             label.new(lastPivot.point, yloc = yloc.price, color = lblColor, style = labelStyle, text = labelText, textcolor = this.properties.textColor, size = size.small, xloc = this.properties.xloc, force_overlay = true):
                             label.new(lastPivot.point, yloc = yloc.price, color = lblColor, style = labelStyle, text = labelText, textcolor = this.properties.textColor, size = size.small, xloc = this.properties.xloc)
                zlabel
...
export method drawplain(ZigzagDrawingPL this) =>
    ...
    this.clear()
    if barstate.islast or barstate.islastconfirmedhistory
        this.zigzagLine := this.properties.force_overlay ? 
                 polyline.new(this.zigzag.getPoints(), this.properties.curved, false, this.properties.xloc, this.properties.lineColor, line_style = this.properties.lineStyle, line_width = this.properties.lineWidth, force_overlay = true) : 
                 polyline.new(this.zigzag.getPoints(), this.properties.curved, false, this.properties.xloc, this.properties.lineColor, line_style = this.properties.lineStyle, line_width = this.properties.lineWidth)
        for i = 0 to this.zigzag.zigzagPivots.size() > 1 ? this.zigzag.zigzagPivots.size() - 2 : na by 1
            lastPivot = this.zigzag.zigzagPivots.get(i)
            llastPivot = this.zigzag.zigzagPivots.get(i + 1)
            ...
            if this.properties.showLabel
                ...
                labelText = (this.properties.showLevels ? '(Level ' + str.tostring(level) + ' : ' + str.tostring(lastPivot.ratio) + '/' + str.tostring(lastPivot.barRatio) + ')' : '') +
                                 (this.properties.showSubcomponents ? lastPivot.subComponents != 0 ? '\nSC :' + str.tostring(lastPivot.subComponents) + '/' : level > 0 ? '\nMC : ' : '' +
                                             (level > 0 ? str.tostring(lastPivot.microComponents) : '') + (not na(lastPivot.subPivots) ? ' (' + subPivotValues.join(',') + ') ' : '') : '')
