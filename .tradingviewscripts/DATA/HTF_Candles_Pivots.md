...
Source: https://www.tradingview.com/script/ooOobeTa-HTF-Candles-Pivots/
...
[image]https://www.tradingview.com/x/seWLxGV4/[/image]
...
var option_hide             = '⊗ Hide'
var option_lang1            = icon_en + ' English'
var option_lang2            = icon_jp + ' 日本語'
...
i_color_name                = str.replace_all(str.lower(input.string('monokai', icon_paint + ' ', options=[
...
i_candle_display            = input.string(option_display1, 'Display ', options=[option_display1, option_display2, option_display3, option_display5, option_display4, option_hide], group=GP3, inline='htf_display')
...
i_pivots_type               = input.string('Traditional', 'Display ', options=['Traditional', 'Fibonacci', 'Woodie', 'Classic', 'DM', 'Camarilla', 'Floor', 'Expected Pivot Points', 'PP only', 'HL2', 'OC2', option_hide], inline='pivots_display', group=GP1)
...
i_pivots_line_style         = f_line_style(input.string(OPTION_BORDER_STYLE2, 'Line  ', options=[OPTION_BORDER_STYLE1, OPTION_BORDER_STYLE2, OPTION_BORDER_STYLE3], inline='pivot_line_style', group=GP1))
...
var message_open            = Message.new('Open', '始値')
var text_w                  = Message.new('W', '週目')
var text_previous           = Message.new('previous', '前日')
var started_at_up           = Message.new('Started at UP', 'アップスタート')
var started_at_down         = Message.new('Started at DOWN', 'ダウンスタート')
    ...
    if _show and (not i_epr_developing)
        ...
        if htf_changed
            if access_shifted or access_all
                ...
                label.new(x1, yc, '◉', textcolor=c_epr, color=TRANSPARENT, style=label.style_label_center, yloc=yloc.price)
...
f_get_cal (_time, _lang, _tz) =>
    ...
    ja = str.format_time(_time, 'yyyy年MM月dd日', _tz) +  '（' + dayOfWeek.getDay(mydayofweek, 1) + '）' + ' ' + myweekofmonth + text_w.get(1)
...
var msg_tds             = Message.new('TDSequential couldn\'t be calculated', 'TDシーケンシャルを計算できませんでした')
var msg_1min            = Message.new('It doesn\'t display correctly on the 1 min.', '1分足では正常に表示できません')
