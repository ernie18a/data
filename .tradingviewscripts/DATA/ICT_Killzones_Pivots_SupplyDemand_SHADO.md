<!-- tradingview-pine-id: PUB;67e9dfaddbf342fa93f719d92ef8345e -->
<!-- tradingviewscripts-format: 1 -->
# ICT Killzones & Pivots + Supply/Demand SHADO

Source: https://www.tradingview.com/script/b83Vo1Z2/
...
Disclaimer 🚨 Visual tool only, no profit guarantee. Test on demo first! ✅
...
````pine
//@version=6
indicator("ICT Killzones & Pivots + Supply/Demand SHADO", "ICT+ZEZE", true, max_labels_count = 500, max_lines_count = 500, max_boxes_count = 500) 
...
// ---------------------------------------- Inputs --------------------------------------------------
var g_KZ            = "Killzones"
show_kz             = input.bool(false, "Show Boxes", tooltip = "Killzone session times follow the Timezone setting in the Global section. Off by default so the colored session boxes don't fight visually with the Supply/Demand zones - turn on if you want them back", group = g_KZ)
max_days            = input.int(1, "Session Limit", 1, tooltip = "Only this many drawings will be kept on the chart, for each selected drawing type (killzone boxes, pivot lines, open lines, range levels, etc.)", group = g_KZ)
...
show_data           = input.bool(false, "Show Data Table", inline = "DATA", tooltip = "Show the most recent ranges of each selected killzone, from high to low", group = g_DATA)
...
dmid                = input.bool(false, "Midline", inline = "DO", tooltip = "Plot the midpoint of the previous day's range - requires High/Low", group = g_DWM)
...
wmid                = input.bool(false, "Midline", inline = "WO", tooltip = "Plot the midpoint of the previous week's range - requires High/Low", group = g_DWM)
...
mmid                = input.bool(false, "Midline", inline = "MO", tooltip = "Plot the midpoint of the previous month's range - requires High/Low", group = g_DWM)
...
show_opens          = input.bool(false, "Show Opening Prices", group = g_OPEN)
...
show_timestamps     = input.bool(false, "Show Timestamps", group = g_VERTICAL)
...
set_table(table tbl, kz kz, array<lvl> levels, int row, string txt, bool use, bool t, color col) =>
    if use and kz._box.size() > 0
        ...
        c += 1
        string range_tip = str.format("Current range is {0,number}, or {1,number,percent} of the {2} ({3,number}) calculated using the last {4} sessions", kz._range_current, pct, measurement, avg, data_lookback)
        if na(avg)
...
// [Data table intentionally removed from output - was cluttering the chart. The
// underlying hit-rate tracking above still runs (harmless/cheap) but nothing renders it]
// ---------------------------------------- Data Table --------------------------------------------------
...
// ============================================================================================================
// Automatic Supply & Demand zone detection layered on top of the killzone/pivot
// framework above. Built for scalping: zones come from short-lookback swing pivots
// and are only kept when the move leaving them clears a volatility filter (ATR
// multiple), which helps filter out noise on lower timeframes (1-5min).
//
// Logic:
//   1. A swing low/high is confirmed once `sd_swing_len` bars pass on both sides
//      without a new low/high (ta.pivotlow / ta.pivothigh).
//   2. The candle at that pivot becomes the "base" - its range (or body) is drawn
//      as a box: green = Demand (support), red = Supply (resistance).
//   3. The zone is only kept if the impulse leaving the base is worth at least
//      `sd_min_move_atr` x ATR - this is the volatility filter that throws out
//      chop/noise pivots and keeps only zones with real order-flow behind them.
//   4. When price returns and wicks (or closes, depending on setting) back into
//      the zone, it is marked "tested" (lighter shading) - first tap is usually
//      the highest-probability scalp entry, later taps are weaker.
//   5. If price closes fully through a zone, it is invalidated (grayed out or
//      removed), since the zone no longer represents unfilled orders.

var g_SD          = "Supply & Demand (Scalping)"
show_sd           = input.bool(true, "Show Supply & Demand Zones", group = g_SD)
sd_swing_len      = input.int(5, "Swing Length (bars each side)", minval = 2, tooltip = "A swing high/low needs this many bars on both sides with a lower high / higher low to confirm a pivot. Smaller = more zones, better for 1-5min scalping charts. Larger = fewer, more significant zones", group = g_SD)
sd_atr_len        = input.int(14, "ATR Length", minval = 1, group = g_SD)
sd_min_move_atr   = input.float(1.2, "Min Impulse Move (x ATR)", minval = 0.1, step = 0.1, tooltip = "The move leaving the pivot must travel at least this many ATRs before the zone is considered valid. Raises this to only keep zones with strong, decisive moves away from them", group = g_SD)
sd_zone_body      = input.string("Wick to Wick", "Zone Boundaries", options = ["Wick to Wick", "Body Only"], tooltip = "Wick to Wick uses the full candle range of the base candle (safer, wider). Body Only uses just the open/close (tighter, more precise entries)", group = g_SD)
sd_mitigation     = input.string("Wick", "Mitigation Trigger", options = ["Wick", "Close"], tooltip = "Wick: the zone is marked 'tested' the first time price wicks into it. Close: only a closing price inside the zone counts as a test", group = g_SD)
sd_invalidate     = input.string("Close Through", "Invalidation", options = ["Close Through", "Never"], tooltip = "Close Through removes/grays a zone once price closes fully through it. Never keeps zones on chart until Max Zones pushes them off", group = g_SD)
sd_max_zones      = input.int(2, "Max Zones Per Side", minval = 1, maxval = 15, tooltip = "Keeps only the most recent N zones per side on chart - lower is cleaner, good for scalping where only the nearest zone matters", group = g_SD)
sd_min_overlap    = input.float(0.3, "Skip If Overlaps Existing Zone (%)", minval = 0.0, maxval = 1.0, step = 0.05, tooltip = "If a new zone would overlap this much (0-1) with an already-active zone on the same side, it's skipped instead of drawn - stops boxes from stacking on top of each other", group = g_SD)
sd_extend_bars    = input.int(0, "Freeze Extension After (bars)", minval = 0, tooltip = "Stops extending an untested zone's right edge after this many bars from creation, to reduce clutter. 0 = keep extending until tested/broken", group = g_SD)
demand_color      = input.color(color.new(color.lime, 75), "Demand Zone", inline = "SDCOL", group = g_SD)
supply_color      = input.color(color.new(color.red, 75), "Supply Zone", inline = "SDCOL", group = g_SD)
sd_alerts         = input.bool(true, "Alert on Zone Tap", group = g_SD)
...
var sd_zone[] demandZones = array.new<sd_zone>()
var sd_zone[] supplyZones = array.new<sd_zone>()

sd_atr = ta.atr(sd_atr_len)

pivLow  = ta.pivotlow(sd_swing_len, sd_swing_len)
pivHigh = ta.pivothigh(sd_swing_len, sd_swing_len)

// checks whether a candidate zone [cTop, cBot] overlaps an existing active zone by more
// than sd_min_overlap (as a fraction of the candidate zone's own size) - used to stop
// new zones from stacking directly on top of ones that already exist
zone_overlaps(sd_zone[] zones, float cTop, float cBot) =>
    bool result = false
    if zones.size() > 0
        for z in zones
            if not z.broken
                float ovTop = math.min(z.top, cTop)
                float ovBot = math.max(z.bottom, cBot)
                float candSize = cTop - cBot
                if ovTop > ovBot and candSize > 0
                    if (ovTop - ovBot) / candSize > sd_min_overlap
                        result := true
    result

// ---- New demand zone off a confirmed swing low ----
if show_sd and not na(pivLow)
    float zTop = sd_zone_body == "Wick to Wick" ? high[sd_swing_len] : math.max(open[sd_swing_len], close[sd_swing_len])
    float zBot = low[sd_swing_len]
    float impulse = high - pivLow
    if impulse >= sd_atr * sd_min_move_atr and not zone_overlaps(demandZones, zTop, zBot)
        int zTime = time[sd_swing_len]
        int zBar  = bar_index[sd_swing_len]
        newBox = box.new(zTime, zTop, time, zBot, xloc = xloc.bar_time, bgcolor = demand_color, border_color = color.new(color.lime, 20), border_width = 1)
        newLbl = label.new(zTime, zBot, "Demand", xloc = xloc.bar_time, style = label.style_label_up, color = transparent, textcolor = color.new(color.lime, 0), size = lbl_size)
        demandZones.unshift(sd_zone.new(newBox, newLbl, zTop, zBot, false, false, zBar, zTime))
        if demandZones.size() > sd_max_zones
            removed = demandZones.pop()
            removed.b.delete()
            removed.lbl.delete()

// ---- New supply zone off a confirmed swing high ----
if show_sd and not na(pivHigh)
    float zTop = high[sd_swing_len]
    float zBot = sd_zone_body == "Wick to Wick" ? low[sd_swing_len] : math.min(open[sd_swing_len], close[sd_swing_len])
    float impulse = pivHigh - low
    if impulse >= sd_atr * sd_min_move_atr and not zone_overlaps(supplyZones, zTop, zBot)
        int zTime = time[sd_swing_len]
        int zBar  = bar_index[sd_swing_len]
        newBox = box.new(zTime, zTop, time, zBot, xloc = xloc.bar_time, bgcolor = supply_color, border_color = color.new(color.red, 20), border_width = 1)
        newLbl = label.new(zTime, zTop, "Supply", xloc = xloc.bar_time, style = label.style_label_down, color = transparent, textcolor = color.new(color.red, 0), size = lbl_size)
        supplyZones.unshift(sd_zone.new(newBox, newLbl, zTop, zBot, false, false, zBar, zTime))
        if supplyZones.size() > sd_max_zones
            removed = supplyZones.pop()
            removed.b.delete()
            removed.lbl.delete()

// ---- Maintain zones each bar: extend, mark tested, invalidate on break ----
manage_sd_side(sd_zone[] zones, bool is_demand) =>
    if zones.size() > 0
        for i = zones.size() - 1 to 0
            z = zones.get(i)
            if not z.broken
                if sd_extend_bars == 0 or (bar_index - z.created_bar) <= sd_extend_bars
                    z.b.set_right(time)

                bool touched = sd_mitigation == "Wick" ? (is_demand ? (low <= z.top and low >= z.bottom) : (high >= z.bottom and high <= z.top)) : (is_demand ? (close <= z.top and close >= z.bottom) : (close >= z.bottom and close <= z.top))
                if touched and not z.tested
                    z.tested := true
                    z.b.set_bgcolor(is_demand ? color.new(color.lime, 87) : color.new(color.red, 87))
                    if sd_alerts
                        alert((is_demand ? "Demand" : "Supply") + " zone tapped - scalp setup", alert.freq_once_per_bar)

                bool brokeThrough = is_demand ? close < z.bottom : close > z.top
                if sd_invalidate == "Close Through" and brokeThrough
                    z.broken := true
                    z.b.set_bgcolor(color.new(color.gray, 92))
                    z.b.set_border_color(color.new(color.gray, 60))
                    z.lbl.set_textcolor(color.new(color.gray, 20))

manage_sd_side(demandZones, true)
manage_sd_side(supplyZones, false)
// ---------------------------------------- Supply & Demand (Scalping) --------------------------------------------------
````
