...
Source: https://www.tradingview.com/script/xZFb6Isb-Smart-Trader-Concentric-Candles-Aristotelian-Cycloids/
...
iEnableFootprint = input.bool(false, "Enable Footprint Engine (Premium/Ultimate Only)", group = grpFP,
     tooltip = "Master toggle for TradingView's tick-level footprint data engine.\n\n⚠️ PLAN REQUIREMENT:\n• request.footprint() is ONLY available on Premium and Ultimate plans.\n• Free, Essential, and Plus plans CANNOT execute this function.\n• If enabled on an unsupported plan, TradingView will throw:\n  'Can't calculate this indicator without pro or expert plan'.\n\nWhen OFF (default):\n• The script never calls request.footprint() — zero plan restriction.\n• CVCA contact analysis still works using geometry + Volume Engine only.\n• Prediction axes 4 (Volume Magnitude), 5 (Delta Character), and\n  6 (Absorption) contribute 0 — prediction relies on axes 1,2,3,7.\n\nWhen ON:\n• Full tick-level footprint data is fetched per bar.\n• All 7 CVCA prediction axes become fully active.\n• Requires Premium ($49.95/mo) or Ultimate plan.\n\nDefault: OFF — safe for all TradingView plans including Free.")

iShowCVCA = input.bool(false, "Show Contact Analysis (CVCA)", group = grpFP,
     tooltip = "Master toggle for Cycloid-Volume Contact Analysis (CVCA).\nThis toggle controls whether request.footprint() is called.\n\n⚠️ PLAN GATE:\n• OFF (default): request.footprint() is NEVER called.\n  Script runs on ALL TradingView plans including Free.\n  Zero plan restriction. CVCA section hidden from dashboard.\n• ON: Activates tick-level footprint data for full 7-axis prediction.\n  Requires TradingView Premium or Ultimate plan.\n  Free, Essential, and Plus plans will produce a runtime error.\n\nWHAT CVCA DOES:\nDetects real-time cycloid curve intersections and applies a 7-axis\nprediction matrix: Direction, Curve Type, Zone, Volume Magnitude,\nDelta Character, Absorption, and Confluence.\nOutputs directional probability P(↑), conviction score, and\nbehavior classification for each active contact.\n\n⚠️ INSTRUMENT NOTE:\n• On high-volatility instruments (BTC, NAS100, etc.), increase\n  'Ticks Per Row' to prevent histogram overflow errors.\n• On stocks and forex with small tick ranges, default settings work.\n\nDefault: OFF — safe for all TradingView plans.")
...
// ENGINE NEED-FLAGS
//──────────────────────────────────────────────────────────────────────────────
// Determines whether each data-request engine must execute on this bar.
// Each flag gates all computation for that engine — zero overhead when OFF.
// Pattern: EP06 Section 7C architecture for clean plan-level gating.
//
// needIntrabar:  true when Volume Engine = "Intrabar (Precise)".
//                Gates tvta.requestUpAndDownVolume() — prevents unnecessary
//                LTF data requests when Geometric mode is selected.
//
// needFootprint: true when CVCA contact analysis is enabled (iShowCVCA).
//                Gates request.footprint() — the ONLY Premium/Ultimate-gated
//                call in the entire script. When false, request.footprint()
//                never executes → script runs on ALL TradingView plans.
//──────────────────────────────────────────────────────────────────────────────

bool needIntrabar  = iCalcMethod == "Intrabar (Precise)"
bool needFootprint = iShowCVCA
...
if needIntrabar
    [_uRaw, _dRaw, _vUnused] = tvta.requestUpAndDownVolume(iLTF)
    u_intra := _uRaw
    d_intra := _dRaw
...
if needFootprint
    fpData := request.footprint(iTicksPerRow, 500)
