...
def _stoch_rsi(values, rsi_length, stoch_length, k_length, d_length):
    ...
    valid = np.isfinite(rsi) & np.isfinite(lowest) & np.isfinite(highest)
    raw[valid] = (rsi[valid] - lowest[valid]) / np.maximum(highest[valid] - lowest[valid], 1e-10) * 100.0
    k = _sma(raw, k_length)
    d = _sma(k, d_length)
    return (k, d)
...
def generate_signals(features, signal_params):
    ...
    primary_macd, primary_signal, primary_hist = _macd(closes)
    _, _, secondary_hist = _macd(closes)
    primary_k, primary_d = _stoch_rsi(closes, 14, 8, 3, 3)
    secondary_k, secondary_d = _stoch_rsi(closes, 7, 7, 3, 2)
    long_entries = np.zeros(size, dtype=np.bool_)
    short_entries = np.zeros(size, dtype=np.bool_)
    if size > 1:
        valid = np.isfinite(primary_macd[1:]) & np.isfinite(primary_signal[1:]) & np.isfinite(primary_hist[1:]) & np.isfinite(primary_hist[:-1]) & np.isfinite(secondary_hist[1:]) & np.isfinite(secondary_hist[:-1]) & np.isfinite(primary_k[1:]) & np.isfinite(primary_d[1:]) & np.isfinite(secondary_k[1:]) & np.isfinite(secondary_d[1:])
        long_entries[1:] = valid & ((primary_macd[1:] > primary_signal[1:]) & (primary_hist[1:] > primary_hist[:-1]) & (secondary_hist[1:] > secondary_hist[:-1]) & (primary_k[1:] > primary_d[1:]) & (secondary_k[1:] > secondary_d[1:]))
        short_entries[1:] = valid & ((primary_macd[1:] < primary_signal[1:]) & (primary_hist[1:] < primary_hist[:-1]) & (secondary_hist[1:] < secondary_hist[:-1]) & (primary_k[1:] < primary_d[1:]) & (secondary_k[1:] < secondary_d[1:]))
...
STRATEGY = {'strategy_id': 'rems_deep_synergy_c3', 'hypothesis': 'Primary MACD 多頭排列、雙週期 MACD 柱體同步上升且雙週期 Stoch RSI 同向時，捕捉多週期動能共振；反向條件捕捉空頭動能。', 'position': 'both', 'generate_signals': generate_signals, 'signal_parameter_names': ['k_base', 'gamma', 'n_base'], 'signal_parameter_sets': [{'k_base': 2.0, 'gamma': 1.0, 'n_base': 2000.0}]}
