...
export const string TT_OPERATIONAL_RANGE_PRIORITY = "Selects the automatic operational R# when multiple retained Ranges compete. Lifecycle activity is always primary: Active > Violating > Breakout Developing. Local Active First (default) prefers a materially contained/local Active Range over an equally Active enclosing Range, then falls back to definitive chronology and structural-completion chronology. Latest Definitive First preserves the prior behavior: equally ranked lifecycle states resolve directly by definitive chronology, then structural completion. This affects operational focus only; it never creates, deletes, validates, archives, reactivates or rewrites a Range."
export const string TT_PRIMARY_RANGE_FOCUS = "Focus-substitution policy only. The operational R# is selected independently by Operational Range Priority. If that R# is Origin-Chop, Pure T↔T First may keep a structurally related open root T↔T Candidate as the effective visual/operational focus. Validated R# First keeps the selected operational R# in focus. This never creates, deletes, validates or reprioritises analytical registry records."
