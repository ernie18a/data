...
//@description XZ Session Authority v3. Canonical suite-wide session-time authority for XZ indicators. Defines deterministic session standards, DST-aware IANA timezones, fixed session segments, custom-session reconstruction, occurrence keys, overlap masks and timestamp-to-active-mask resolution. It deliberately contains no indicator-specific lifecycle, qualification, Supply/Demand, FVG, IL, market-structure, Auction Engine or Research methodology.
library("XZ_Session_Authority", false)

//==============================================================================
// XZ SESSION AUTHORITY v3
//==============================================================================
// v3 adds generic selected-market helpers so consumers do not reimplement the same
// Sydney/Tokyo/London/New York selection mask/count logic.
...
//@function Returns whether one canonical session index is enabled by caller-supplied market selection toggles.
export marketSelected(int sessionIndex, bool showSydney, bool showTokyo, bool showLondon, bool showNewYork) =>
    sessionIndex == 0 ? showSydney :
     sessionIndex == 1 ? showTokyo :
     sessionIndex == 2 ? showLondon :
     showNewYork

//@function Packs caller-supplied Sydney/Tokyo/London/New York selection toggles into the canonical four-session mask.
export marketSelectionMask(bool showSydney, bool showTokyo, bool showLondon, bool showNewYork) =>
    int result = 0
    if showSydney
        result := maskAdd(result, 0)
    if showTokyo
        result := maskAdd(result, 1)
    if showLondon
        result := maskAdd(result, 2)
    if showNewYork
        result := maskAdd(result, 3)
    result

//@function Counts caller-selected canonical market sessions.
export marketSelectionCount(bool showSydney, bool showTokyo, bool showLondon, bool showNewYork) =>
    maskCount(marketSelectionMask(showSydney, showTokyo, showLondon, showNewYork))
