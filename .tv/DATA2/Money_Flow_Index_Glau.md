...
// Plot
plot(mf, "MFI Bull / Bear", color=colorMFI, linewidth=2)

// Níveis ajustáveis
midLine = 50.0
upLine = input.int(80, "Upper Line",tooltip= "60 - 100", minval=60, maxval=100, display=display.none)
lowLine = input.int(20, "Lower Line",tooltip= "0 - 40", minval= 0, maxval=40, display=display.none)
...
fill(pUp, pMid, color=color.new(color.orange, 75), title="Resistance")
fill(pLow, pMid, color=color.new(color.blue, 75), title="Support")
...
// 2 Alertas Time Real
alertcondition(mfiUpRT, title="MFI Bull — Real Time", message="MFI Bull - Real time.")
alertcondition(mfiDownRT, title="MFI Bear — Real Time", message="MFI Bear - Real time.")

// 2 Alertas Past
alertcondition(mfiUpPast, title="MFI Bull — Confirmed", message="MFI Bull — candle confirmed.")
alertcondition(mfiDownPast, title="MFI Bear — Confirmed", message="MFI Bear — candle confirmed.")
````
