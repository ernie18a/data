...
// =============================================================================
// SECTION 2: USER INPUTS - STREAMLINED & INSTITUTIONAL (EDITOR'S PICK STANDARD)
// =============================================================================

// --- 1. Execution Engine & Performance ---
grp_engine = "1. Execution Engine & Performance"
replayOptimized         = input.bool(true, "⚡ Fast Replay / Lightweight Mode (Universal Free & Premium)", group=grp_engine,
...
engineInput             = input.string("Native Footprint (Tick Precision)", "Order Flow Engine (when Lightweight Disabled)",
     options=["Native Footprint (Tick Precision)", "Geometric Proxy (Zero Lag / Replay)", "Intrabar (1m Low Memory)", "Intrabar (Sub-Minute)"], group=grp_engine,
...
effortSensitivity       = input.int(5, "Institutional Effort Sensitivity (1 - 10)", minval=1, maxval=10, group=grp_engine,
...
htfClutterFilter        = input.bool(true, "Auto-Suppress Intraday Overlays on Macro/HTF", group=grp_engine,
...
// --- 2. Order Flow Signals & Footprint Analytics ---
grp_orderflow = "2. Order Flow Signals & Footprint Analytics"
showEffortReward        = input.bool(true, "Color Effort vs. Reward Candles", group=grp_orderflow,
...
candleStyle             = input.string("Ghost Glow (Semi-Transparent Body + Solid Wicks)", "Candle Display Style",
     options=["Ghost Glow (Semi-Transparent Body + Solid Wicks)", "Full Solid HD Candlestick", "Hollow (Wicks & Solid Borders Only)", "Direct Native Barcolor", "Disabled"], group=grp_orderflow,
     tooltip="Selects visual presentation style for price candles:\n• Ghost Glow: Semi-transparent bodies with solid wicks, engineered specifically to overlay cleanly on native Volume Footprint charts without obscuring numbers.\n• Full Solid HD: Traditional high-contrast solid candlesticks.\n• Hollow: Wicks and solid borders only with transparent bodies.\n• Direct Native Barcolor: Native chart candle coloring.\n• Disabled: Suppresses candle coloring completely.")
...
shelfBoxTransp          = input.int(84, "Imbalance Shelf Box Transp (%)", minval=0, maxval=100, group=grp_orderflow,
     tooltip="Controls the fill transparency of stacked imbalance support/demand shelf boxes (0% = solid, 84% = translucent institutional shading, 100% = hidden).")
...
showUnfinishedAuctions  = input.bool(true, "Unfinished Auction Magnets (Poor Highs/Lows)", group=grp_orderflow,
     tooltip="Projects horizontal dotted rays from candle extremes where volume executed at the outer tick (dalton poor highs and poor lows serving as high-probability retest targets).")

// --- 3. Auction Market Theory & Session Profiles ---
grp_amt = "3. Auction Market Theory & Session Profiles"
showSessionProfiles     = input.bool(true, "Show Session Volume Profiles (Right Margin & Anchor)", group=grp_amt,
...
profWidthModeInput      = input.string("Auto-Adaptive (Viewport & TF Scaled)", "Developing Profile Width",
     options=["Auto-Adaptive (Viewport & TF Scaled)", "Fixed Compact (18 Bars)", "Fixed Balanced (24 Bars)", "Custom Bars"], group=grp_amt,
...
profCustomWidthInput    = input.int(20, "Custom Profile Width (Bars)", minval=8, maxval=40, group=grp_amt,
...
profTimeframeMode       = input.string("Auto-Adaptive", "Profile Scope Horizon",
     options=["Auto-Adaptive", "Sub-Session (Cash/ON)", "Daily Full Cycle (24H)", "Weekly (Mon-Fri)", "Monthly"], group=grp_amt,
...
priorSessionsCount      = input.int(15, "Completed Sessions to Display", minval=1, maxval=35, group=grp_amt,
     tooltip="Number of historical completed session volume profiles to render and keep active in memory.")
extendVpocInput         = input.bool(true, "Extend Virgin POC to Live Bar", group=grp_amt,
     tooltip="Projects a horizontal dotted line forward from the most recent completed session VPOC until the live bar.")
showSessionVa           = input.bool(true, "Show Session VAH / VAL Lines", group=grp_amt,
     tooltip="Plots Value Area High (VAH) and Value Area Low (VAL) boundary lines on session profiles.")
showAmtShapes           = input.bool(true, "Show Dalton Profile Shape Badges ([D], [P], [b], [B])", group=grp_amt,
...
showOvlMetrics          = input.bool(true, "Show Ata Sabanci Overlap Metrics (OVL & Tilt %)", group=grp_amt,
     tooltip="Displays Ata Sabanci profile overlap coefficient (OVL %) and directional volume tilt on session labels and HUD.")
showLvnCorridor         = input.bool(true, "Show LVN Vacuum Corridor (B-Shape)", group=grp_amt,
     tooltip="Detects and highlights Low Volume Nodes (LVN) liquidity vacuum corridors on B-Shape double distribution profiles.")
showSessionFrames       = input.bool(true, "Show Subsession Framing Guidelines", group=grp_amt,
...
frameGeometryMode       = input.string("Ceiling & Floor Guidelines (No Vertical Lines)", "Session Framing Geometry",
     options=["Ceiling & Floor Guidelines (No Vertical Lines)", "Full Enclosing Box", "None (Disabled)"], group=grp_amt,
...
frameOutlineStyleChoice = input.string("Dotted", "Framing Line Style", options=["Solid", "Dotted", "Dashed"], inline="FrameLine", group=grp_amt)
frameOutlineWidth       = input.int(1, "Width", minval=1, maxval=4, inline="FrameLine", group=grp_amt,
     tooltip="Customizes line style (Solid/Dotted/Dashed) and pixel width for session framing boundaries.")

// --- 4. Key Structural Levels & Confluence ---
grp_levels = "4. Key Structural Levels & Confluence"
showPriorCashLevels     = input.bool(true, "Show Prior Day Cash Levels (VAH / VAL / VPOC)", group=grp_levels,
     tooltip="Displays Prior Day RTH Cash (09:30-16:00 EST) Value Area (VAH / VAL) and Point of Control (VPOC).")
showHalfback            = input.bool(true, "Show Overnight Half-Back (50% Midpoint)", group=grp_levels,
     tooltip="Projects the exact 50% midpoint of the overnight auction (Globex ONH to ONL).")
...
// --- 5. Institutional Visual Palette & Styling ---
grp_palette = "5. Institutional Visual Palette & Styling"
colBuyerAbsorb          = input.color(#FFD700, "Buyer Absorption (Demand at Lows)", group=grp_palette,
...
colSellerAbsorb         = input.color(#AB47BC, "Seller Absorption (Supply at Highs)", group=grp_palette,
...
colBullDrive            = input.color(#00E676, "Initiative Bull Drive (Bright Green)", group=grp_palette,
     tooltip="High-volume initiative buying expansion drive (electric neon green).")
colBearDrive            = input.color(#FF1744, "Initiative Bear Drive (Bright Red)", group=grp_palette,
     tooltip="High-volume initiative selling expansion drive (electric neon crimson).")
colBullCandle           = input.color(#26A69A, "Standard Bull Bar (Muted)", group=grp_palette,
     tooltip="Standard non-initiative bullish candle body/wick color (muted low-fatigue green).")
colBearCandle           = input.color(#EF5350, "Standard Bear Bar (Muted)", group=grp_palette,
     tooltip="Standard non-initiative bearish candle body/wick color (muted low-fatigue red).")
colMutedNoise           = input.color(#373C4B, "Dead Volume / Micro-Chop", group=grp_palette,
     tooltip="Low relative volume or micro-range consolidation candle. Customizable for dark or white chart backgrounds.")
colPocInput             = input.color(#FFFFFF, "VPOC Color", group=grp_palette,
     tooltip="Color of the Point of Control (POC / VPOC) line, marking the exact price of heaviest volume execution.")
colVahInput             = input.color(#00E5FF, "VAH & VAL Boundary Color", group=grp_palette,
...
halfbackColInput        = input.color(#FF9800, "Overnight Half-Back Color", group=grp_palette,
     tooltip="Color of the Overnight Half-Back (50% midpoint) line.")
confluentColInput       = input.color(#CE93D8, "Confluent POC Color", group=grp_palette,
     tooltip="Color of the Confluent POC level when Prior Cash VPOC and Overnight VPOC align closely.")
colLvnInput             = input.color(#E040FB, "LVN Color", group=grp_palette,
...
profShadingMode         = input.string("Bid/Ask Dual-Tone", "Profile Shading Style",
     options=["Bid/Ask Dual-Tone", "Monochrome Slate", "Monochrome Gold", "Monochrome Cyan"], group=grp_palette,
...
profBuyCol              = input.color(#FFD700, "Buy Vol Profile Color", group=grp_palette,
     tooltip="Volume profile color for buying volume (defaults to Gold #FFD700). Fully customizable.")
profSellCol             = input.color(#AB47BC, "Sell Vol Profile Color", group=grp_palette,
     tooltip="Volume profile color for selling volume (defaults to Purple #AB47BC). Fully customizable.")
profFillTransp          = input.int(75, "Profile Fill Transp (%)", minval=0, maxval=100, group=grp_palette,
     tooltip="Controls fill opacity of the volume profile curves across all sessions (70-75% provides rich institutional dual-tone overlap shading, 100% = borderless/outline only).")
showProfOutline         = input.bool(true, "Profile Outline Contour", inline="ProfOut", group=grp_palette)
profOutlineWidth        = input.int(1, "Width", minval=1, maxval=4, inline="ProfOut", group=grp_palette,
     tooltip="Toggle outline border and customize thickness (1-4px) for developing and completed volume profile polyline curves.")
cashFrameCol            = input.color(#00E5FF, "Cash Frame Color", group=grp_palette,
     tooltip="Border color for Cash RTH (09:30-16:00 EST) subsession framing outlines.")
onFrameCol              = input.color(#AB47BC, "ON Frame Color", group=grp_palette,
     tooltip="Border color for Overnight Globex (18:00-09:30 EST) subsession framing outlines.")
frameOutlineTransp      = input.int(30, "Subsession Frame Transp (%)", minval=0, maxval=100, group=grp_palette,
     tooltip="Controls opacity/transparency of the subsession frame borders (0% = solid, 30% = crisp dotted, 100% = completely hidden).")
frameFillTransp         = input.int(100, "Subsession Fill Transp (%)", minval=0, maxval=100, group=grp_palette,
...
// --- 6. Heads-Up Display & Real-Time Telemetry ---
grp_hud = "6. Heads-Up Display & Real-Time Telemetry"
...
// (Automated Internal Engine — Hardcoded to eliminate user interface clutter)
// ─────────────────────────────────────────────────────────────────────────────
...
string priorProfPlacement      = "Pin Overnight/Prior to Visible Left Margin (LTF Cockpit)" // Dynamic Adaptive Viewport
int    leftProfWidthInput      = 24
...
// Purged Overnight Shaded Box Internal Defaults (Zero-Visual Impact)
...
// Conformal Dimensionless Absorption & Microstructure Effort Calibration:
// Calibrated from 16.2M CME Level 3 MBO tick dataset (P50=1.00, P75=1.25, P90=1.50, P95=1.75)
...
// Fast 50% Wick Reclaim Gate (Pattern 9 Asymmetry Trap Defense)
...
bool fp_unfinished_high     = false
bool fp_unfinished_low      = false
...
if useFpEngine and hasEngFp and (showEffortReward or showImbalanceShelves or showUnfinishedAuctions)
    ...
    if num_fp_rows >= 2
        ...
        // 1. True Unfinished Auction Verification at Extreme Bid/Ask Tiers
        volume_row r_top = array.get(fp_rows, num_fp_rows - 1)
        float b_top = volume_row.buy_volume(r_top)
        float s_top = volume_row.sell_volume(r_top)
        if b_top > 0.0 and s_top > 0.0 and (b_top + s_top >= min_imb_vol)
            fp_unfinished_high := true

        volume_row r_bot = array.get(fp_rows, 0)
        float b_bot = volume_row.buy_volume(r_bot)
        float s_bot = volume_row.sell_volume(r_bot)
        if b_bot > 0.0 and s_bot > 0.0 and (b_bot + s_bot >= min_imb_vol)
            fp_unfinished_low := true

    // 2. Intra-Candle Point of Control (POC) Trapped Liquidity Sentry
...
color bull_wick_col = is_bull_drive ? colBullDrive : colBullCandle
color bear_wick_col = is_bear_drive ? colBearDrive : colBearCandle
...
plotcandle(is_bull_state ? open : na, is_bull_state ? high : na, is_bull_state ? low : na, is_bull_state ? close : na,
     ...
     color=color.new(bull_wick_col, is_bull_drive ? math.min(body_transp, 20) : body_transp),
...
plotcandle(is_bear_state ? open : na, is_bear_state ? high : na, is_bear_state ? low : na, is_bear_state ? close : na,
     ...
     color=color.new(bear_wick_col, is_bear_drive ? math.min(body_transp, 20) : body_transp),
...
bool is_poor_high = false
bool is_poor_low  = false

if showUnfinishedAuctions and not hide_intraday_overlays
    // Check if candle extreme printed two-sided volume at outer tick without exhaustion (no zero print)
    // 1. Native Footprint Engine: True bid/ask liquidity at extreme price rows
    // 2. Intrabar / Geo Proxy Fallback: Extreme wick < 10% with high volume at outer edge
    is_poor_high := (useFpEngine and hasEngFp) ? fp_unfinished_high : (upper_wick_pct <= 0.10 and effective_rvol >= 1.10 and close > open)
    is_poor_low  := (useFpEngine and hasEngFp) ? fp_unfinished_low  : (lower_wick_pct <= 0.10 and effective_rvol >= 1.10 and close < open)
    ...
    // Extend and check repairs (unfinished auction retest)
...
bool is_left_pinned = (priorProfPlacement != "Session Origin (Standard)")
bool no_proj_lines = is_left_pinned or (priorLevelAnchor == "Session Framing Only (No Multi-Day Lines)")
if no_proj_lines
...
// Viewport-Adaptive Visible Screen Margin Detection (TradingView Visible Chart Objects)
var int left_visible_bar_index = na
if barstate.isfirst
    left_visible_bar_index := na
if na(left_visible_bar_index) and not na(chart.left_visible_bar_time) and (time >= chart.left_visible_bar_time)
    left_visible_bar_index := bar_index

int bar_span_ms = int(math.max(1000, time - nz(time[1], time)))
int approx_left_vis = not na(chart.left_visible_bar_time) ? math.max(0, bar_index - int(math.round((time - chart.left_visible_bar_time) / bar_span_ms))) : na
int eff_left_vis_idx = (not na(left_visible_bar_index) and left_visible_bar_index < bar_index) ? left_visible_bar_index : (not na(approx_left_vis) ? approx_left_vis : left_visible_bar_index)
...
var array<int>   prior_cadence_cache = array.new<int>(4, -1)   // [0: prior_sess_bar, 1: is_active (1 or 0), 2: left_vis_idx, 3: vis_state (1: out of view, 0: in view)]
...
if barstate.islast and showSessionProfiles and array.size(sess_starts) > 0 and not suppress_profile
        ...
        if need_dev_recalc
            ...
            if dev_span > 0.0 and dev_tot_v > 0.0
                ...
                if vMax_dev > 0.0
                    ...
                    dev_poc_line := line.new(bar_index, dev_poc_px, r_edge_dev, dev_poc_px,
                    ...
                    if showSessionVa
                        dev_vah_line := line.new(bar_index, dev_vah_px, r_edge_dev, dev_vah_px,
                             color=COL_VAH_LINE, width=vahLineWidth, style=STYLE_VAH_LINE)
                        dev_val_line := line.new(bar_index, dev_val_px, r_edge_dev, dev_val_px,
                    ...
                    if showAmtShapes
                        ...
                        dev_shape_lbl := label.new(r_edge_dev, dev_hi, dev_shape_txt,
            ...
            if not na(dev_shape_lbl)
                label.set_x(dev_shape_lbl, r_edge_dev)
            if not na(dev_vah_line)
    ...
    if array.size(sess_starts) >= min_sess_req
        ...
        bool is_prior_out_of_view = not na(eff_left_vis_idx) and (latest_prior_start < eff_left_vis_idx)
        int cur_vis_state = is_prior_out_of_view ? 1 : 0
        bool render_latest_on_left = (priorProfPlacement == "Both (Origin + Left Margin)") or (priorProfPlacement == "Pin Overnight/Prior to Visible Left Margin (LTF Cockpit)" and is_prior_out_of_view)

        // Cadence Gate for Prior Sessions: Recompute on session handover, active state change, or viewport scroll
        ...
        int c_last_left_vis   = array.size(prior_cadence_cache) > 2 ? array.get(prior_cadence_cache, 2) : -1
        int c_last_vis_state  = array.size(prior_cadence_cache) > 3 ? array.get(prior_cadence_cache, 3) : -1
        int cur_act_int       = is_cur_sess_active ? 1 : 0
        int cur_left_vis      = nz(eff_left_vis_idx, -1)
        bool need_scroll_recalc = (render_latest_on_left and cur_left_vis != c_last_left_vis) or (cur_vis_state != c_last_vis_state)
        bool need_prior_recalc = (array.size(prior_buy_polys) == 0) or (latest_prior_start != c_last_prior_sess) or (cur_act_int != c_last_active) or need_scroll_recalc
        ...
        if need_prior_recalc
            ...
            int base_left_x = not na(eff_left_vis_idx) ? eff_left_vis_idx + 1 : array.get(sess_starts, latest_closed_idx)
            int eff_w_left  = leftProfWidthInput
            ...
            for s_idx = s_start_idx to max_prior_sessions
                ...
                if prior_len >= 2 and p_i_max >= p_i_min
                    ...
                    if prior_span > 0.0 and prior_tot_v > 0.0
                        ...
                        // Render Completed Session Profile Anchored at Session Start and/or Visible Left Margin
                        bool is_latest_prior = (s_idx == latest_closed_idx)
                        bool render_origin_profile = (priorProfPlacement == "Session Origin (Standard)") or (priorProfPlacement == "Both (Origin + Left Margin)") or (not is_latest_prior) or (is_latest_prior and not is_prior_out_of_view)
                        bool render_left_margin_profile = is_latest_prior and render_latest_on_left
                        ...
                        if vMax_prior > 0.0
                            ...
                            // 1. Session Origin Projection
                            if render_origin_profile
                            ...
                            // 2. Visible Left Margin Projection (Adaptive Intraday Cockpit for Lower Timeframes)
                            if render_left_margin_profile
                                array<chart.point> ptsB_l = array.new<chart.point>()
                                array.push(ptsB_l, chart.point.from_index(base_left_x, prior_lo))
                                for b = 0 to res_p
                                    int xb = base_left_x + int(math.round(eff_w_left * array.get(b_prior, b) / vMax_prior))
                                    array.push(ptsB_l, chart.point.from_index(xb, prior_lo + b * step_p))
                                array.push(ptsB_l, chart.point.from_index(base_left_x, prior_hi))
                                polyline p_buy_l = polyline.new(ptsB_l, curved=false, closed=true,
                                     line_color=prior_b_line, fill_color=prior_b_fill, line_width=profOutlineWidth)
                                array.push(prior_buy_polys, p_buy_l)

                                array<chart.point> ptsS_l = array.new<chart.point>()
                                array.push(ptsS_l, chart.point.from_index(base_left_x, prior_lo))
                                for b = 0 to res_p
                                    int xs = base_left_x + int(math.round(eff_w_left * array.get(s_prior, b) / vMax_prior))
                                    array.push(ptsS_l, chart.point.from_index(xs, prior_lo + b * step_p))
                                array.push(ptsS_l, chart.point.from_index(base_left_x, prior_hi))
                                polyline p_sell_l = polyline.new(ptsS_l, curved=false, closed=true,
                                     line_color=prior_s_line, fill_color=prior_s_fill, line_width=profOutlineWidth)
                                array.push(prior_sell_polys, p_sell_l)
                            ...
                            if showSessionVa
                                int vah_x1 = render_left_margin_profile ? base_left_x : prior_start_bar
                                int vah_x2 = render_left_margin_profile ? (base_left_x + eff_w_left) : prior_end_bar
                                ...
                                if levelLabelStyle != "Hidden"
                                    int p_lbl_x = render_left_margin_profile ? (base_left_x + eff_w_left + 2) :
                                         (levelLabelPos == "Left Edge (Inside Box)" ? prior_start_bar :
                                    ...
                                    string p_lbl_st = render_left_margin_profile ? label.style_label_left :
                                         (levelLabelStyle == "Bulky Badges (Legacy)" ? label.style_label_right :
                                    ...
                                    int eff_lbl_x = (not render_left_margin_profile and levelLabelStyle == "Bulky Badges (Legacy)") ? prior_start_bar : p_lbl_x
                            ...
                            if showHalfback and not prior_is_cash and not is_daily_cycle_mode and not is_weekly_mode and not is_monthly_mode
                                bool is_covered_by_live = (is_cur_sess_active and in_cash_rth and s_idx == 1 and not na(hb_line_inst)) and not render_left_margin_profile
                                if not is_covered_by_live
                                    float prior_hb_px = (prior_hi + prior_lo) * 0.5
                                    int hb_start = render_left_margin_profile ? base_left_x : prior_start_bar
                                    int hb_end   = render_left_margin_profile ? (base_left_x + eff_w_left) : prior_end_bar
                                    ...
                                    if levelLabelStyle != "Hidden"
                                        int p_hb_lbl_x = render_left_margin_profile ? (base_left_x + eff_w_left + 2) :
                                             (levelLabelPos == "Left Edge (Inside Box)" ? hb_start :
                                             (levelLabelPos == "Center of Session" ? int(math.round((hb_start + hb_end) * 0.5)) : (levelLabelPos == "Right Edge (Outside Box)" ? hb_end + 1 : hb_end)))
                                        string p_hb_lbl_st = render_left_margin_profile ? label.style_label_left :
                                             (levelLabelStyle == "Bulky Badges (Legacy)" ? label.style_label_right :
                                             (levelLabelPos == "Center of Session" ? label.style_label_center :
                                             (levelLabelPos == "Right Edge (Inside Box)" ? label.style_label_right : label.style_label_left)))
                                        int eff_hb_lbl_x = (not render_left_margin_profile and levelLabelStyle == "Bulky Badges (Legacy)") ? hb_start : p_hb_lbl_x
                            ...
                            if is_prior_b_shape and showLvnCorridor and not na(prior_lvn_px)
                                int r_edge_lvn = render_left_margin_profile ? (base_left_x + eff_w_left) : prior_end_bar
                                if not render_left_margin_profile and lvnExtendMode == "Extend Until Mitigated" and priorLevelAnchor != "Session Framing Only (No Multi-Day Lines)"
                                    if p_i_min > 0
                                        int mit_i = -1
                                        int f_i = p_i_min - 1
                                        while f_i >= 0
                                ...
                                int lvn_x1 = render_left_margin_profile ? base_left_x : prior_start_bar
                                ...
                                if levelLabelStyle != "Hidden"
                                    int p_lvn_lbl_x = render_left_margin_profile ? (base_left_x + eff_w_left + 2) :
                                         (levelLabelPos == "Left Edge (Inside Box)" ? prior_start_bar :
                                         (levelLabelPos == "Center of Session" ? int(math.round((prior_start_bar + prior_end_bar) * 0.5)) : (levelLabelPos == "Right Edge (Outside Box)" ? prior_end_bar + 1 : prior_end_bar)))
                                    string p_lvn_lbl_st = render_left_margin_profile ? label.style_label_left :
                                         (levelLabelStyle == "Bulky Badges (Legacy)" ? label.style_label_right :
                                         (levelLabelPos == "Center of Session" ? label.style_label_center :
                                         (levelLabelPos == "Right Edge (Inside Box)" ? label.style_label_right : label.style_label_left)))
                                    int eff_lvn_lbl_x = (not render_left_margin_profile and levelLabelStyle == "Bulky Badges (Legacy)") ? prior_start_bar : p_lvn_lbl_x
                            ...
                            if showAmtShapes
                                ...
                                int eff_shape_x = render_left_margin_profile ? (base_left_x + eff_w_left + 2) : (prior_start_bar + eff_w_prior + 2)
            ...
            for k = 0 to (n_pocs > 0 ? n_pocs - 1 : -1)
                ...
                bool is_k_left = (k_s_idx == latest_closed_idx) and render_latest_on_left
                int eff_k_start = is_k_left ? base_left_x : k_start
                int eff_k_end   = is_k_left ? (base_left_x + eff_w_left) : k_end
                ...
                if is_k_left and priorProfPlacement == "Both (Origin + Left Margin)"
                    line p_poc_orig = line.new(k_start, k_px, k_end, k_px,
                         color=eff_poc_col, width=eff_poc_w, style=eff_poc_st)
                    array.push(prior_poc_lines, p_poc_orig)

                string k_pfx = is_weekly_mode ? "📅 W-" : (is_monthly_mode ? "🗓️ M-" : (is_daily_cycle_mode ? "🌐 D-" : (k_is_cash ? (is_crypto_asset ? "☀️ DAY " : "☀️ CASH ") : (is_crypto_asset ? "🌙 NIGHT " : "🌙 ON "))))

                if levelLabelStyle != "Hidden"
                    int p_lbl_x = is_k_left ? (base_left_x + eff_w_left + 2) :
                         (levelLabelPos == "Left Edge (Inside Box)" ? k_start :
                         (levelLabelPos == "Center of Session" ? int(math.round((k_start + k_end) * 0.5)) : (levelLabelPos == "Right Edge (Outside Box)" ? k_end + 1 : k_end)))
                    string p_lbl_st = is_k_left ? label.style_label_left :
                         (levelLabelStyle == "Bulky Badges (Legacy)" ? label.style_label_right :
                         (levelLabelPos == "Center of Session" ? label.style_label_center :
                         (levelLabelPos == "Right Edge (Inside Box)" ? label.style_label_right : label.style_label_left)))
                    int eff_lbl_x = (not is_k_left and levelLabelStyle == "Bulky Badges (Legacy)") ? k_start : p_lbl_x
                ...
                if extendVpocInput and k_s_idx == latest_closed_idx and not is_k_left and priorLevelAnchor != "Session Framing Only (No Multi-Day Lines)"
            ...
            array.set(prior_cadence_cache, 2, cur_left_vis)
            array.set(prior_cadence_cache, 3, cur_vis_state)
        else
...
if barstate.islast and hudModeInput != "Disabled"
    ...
    string bar_delta_str  = (bar_delta >= 0 ? "+" : "") + str.tostring(bar_delta, "#,###") + " Δ (" + str.tostring(delta_pct, "0.0") + "%)"
    ...
    if hudModeInput == "Compact Status Pill"
        ...
        table.cell(hud_display, 2, 0, active_engine_label + " | RVOL: " + str.tostring(effective_rvol, "#.#") + "x (" + (effective_rvol >= 2.0 ? "High Vol" : (effective_rvol >= 1.2 ? "Active" : "Normal")) + ") | " + (is_weekly_mode ? "Weekly" : (is_monthly_mode ? "Monthly" : (is_daily_or_above ? "Macro" : "Sub-Sess"))), text_color=color.rgb(176, 190, 197), text_size=size.small, bgcolor=bg_hdr)
        ...
        string flow_interp = is_bull_delta_div ? "⚡ Bullish Delta Divergence (Trapped Sellers)" :
             (is_bear_delta_div ? "⚡ Bearish Delta Divergence (Trapped Buyers)" :
             (fp_bottom_sell_imb ? "⚡ Footprint Iceberg Bid at Lows" :
             (fp_top_buy_imb ? "⚡ Footprint Iceberg Ask at Highs" :
             (fp_trapped_shorts ? "⚡ Trapped Shorts at Lows (Demand Absorption)" :
             (fp_trapped_longs ? "⚡ Trapped Longs at Highs (Supply Absorption)" :
             (std_bull_absorb ? "⚡ Institutional Demand Absorption (Passive Bids)" :
             (std_bear_absorb ? "⚡ Institutional Supply Defense (Passive Asks)" :
             (is_gold_buyer_absorb ? "⚡ Buyer Absorption Active at Lows" :
             (is_purple_seller_absorb ? "⚡ Seller Defense Active at Highs" :
             (is_bull_drive ? "Initiative Bull Drive" :
             (is_bear_drive ? "Initiative Bear Drive" : "Balanced Tape Delta")))))))))))
        ...
        table.cell(hud_display, 0, 6, "🎓 AMT Theory", text_color=color.rgb(176, 190, 197), text_size=size.tiny, bgcolor=bg_alt)
...
bool alert_poor     = showUnfinishedAuctions and barstate.isconfirmed and (is_poor_high or is_poor_low)
...
alertcondition(alert_poor,     title="Unfinished Auction Magnet",      message="[CME LENS] True unfinished auction (poor high/low) detected with active liquidity magnet.")
