...
length8 = input.int(8, title="MA-8 period", minval=1, group=grpRibbon)
...
plot(ma8, title="MA-8", color=color.yellow, linewidth=2, offset=offsetInput)
...
// ====== تنبيه (Alert): فتح شمعة فوق/تحت خط السيولة العالية (شراء/بيع) ======
// يتحقق الشرط عندما تفتح شمعة جديدة فوق خط السيولة (إشارة شراء) أو تحته
// (إشارة بيع)، مقارنة بموقع فتح الشمعة السابقة من نفس الخط.
liqBuySignal  = not na(stored_midpoint) and ta.crossover(open, stored_midpoint)
liqSellSignal = not na(stored_midpoint) and ta.crossunder(open, stored_midpoint)

// إشارات بصرية على الشارت
plotshape(liqBuySignal, title="Liquidity Level Buy", style=shape.arrowup, location=location.belowbar,
     color=color.lime, size=size.tiny)
plotshape(liqSellSignal, title="Liquidity Level Sell", style=shape.arrowdown, location=location.abovebar,
     color=color.red, size=size.tiny)

// فعّلها من قائمة Create Alert واختر شرط "Liquidity Level Breakout Up (Buy)"
// أو "Liquidity Level Breakout Down (Sell)" من هذا المؤشر.
alertcondition(liqBuySignal, title="Liquidity Level Breakout Up (Buy)",
     message="{{ticker}} ({{interval}}): فتحت شمعة فوق خط السيولة العالية (شراء)")
alertcondition(liqSellSignal, title="Liquidity Level Breakout Down (Sell)",
     message="{{ticker}} ({{interval}}): فتحت شمعة تحت خط السيولة العالية (بيع)")

// ==========================================================================
// ============================  4) Golden Zone  ============================
// ==========================================================================
// نسخة مبسّطة ومستقلة لفكرة "المنطقة الذهبية": نكتشف آخر قمة وقاع (Pivot)
// تلقائياً حسب عدد شموع محدد من كل جهة، نحسب الفيبوناتشي بينهم، ونلوّن
// منطقة الارتداد بين مستويي 0.5 و0.618 كمنطقة ذهبية. الخط يتحدّث ويمتد
// تلقائياً لليمين بدون تكرار رسم أو تكدّس كائنات على الشارت.
...
// حساب مستوى فيبوناتشي معين v بين آخر قمة وقاع، مع مراعاة اتجاه الحركة:
// لو القاع أقدم من القمة (حركة صاعدة) يُقاس الارتداد من القمة نزولاً،
// ولو القمة أقدم من القاع (حركة هابطة) يُقاس الارتداد من القاع صعوداً.
gzFib(v) =>
    float result = na
