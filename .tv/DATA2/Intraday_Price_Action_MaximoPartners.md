...
//@version=6
indicator(
     title            = "Intraday Price Action [MaximoPartners]",
...
// INPUTS
//══════════════════════════════════════
string zonesGroup = "BUY/SELLERS BEHAVIOUR"
string levelsGroup = "Reference Levels"
string indicatorsGroup = "Indicators"
string displayGroup = "Display"

bool showVolumeMovesInput = input.bool(true, "Volume-dominance moves", group = zonesGroup)
bool showDestroyedZonesInput = input.bool(true, "invalidated zones", group = zonesGroup)
bool showLiquidationsInput = input.bool(true, "liquidity levels", group = zonesGroup)

bool showAsiaOrbInput = input.bool(true, "Tokyo opening range", group = levelsGroup)
bool showLondonOrbInput = input.bool(true, "London opening range", group = levelsGroup)
bool showNewYorkOrbInput = input.bool(true, "New York opening range", group = levelsGroup)

bool showPreviousLevelsInput = input.bool(true, "Previous highs/lows", group = levelsGroup)

bool showVwapInput = input.bool(true, "VWAP Daily", group = indicatorsGroup)
bool showSessionVwapInput = input.bool(true, "VWAP Session", group = indicatorsGroup)
bool showEmasInput = input.bool(true, "EMAs", group = indicatorsGroup)

bool intradayOnlyInput = true
...
if newDay or barstate.isfirst
    ...
    if showPreviousLevelsInput and f_visible_by_time(prevDayInsightTime)
...
if newWeek or barstate.isfirst
    ...
    if showPreviousLevelsInput and insightsAllowed
...
//══════════════════════════════════════
// VWAP SESSION - RESET AT EACH MARKET OPEN
// One VWAP accumulates continuously until the next Tokyo, London, or NY open.
// Local opening times match the ORB schedules; IANA zones handle DST.
// On unaligned timeframes, reset on the first bar opening at/after the open.
//══════════════════════════════════════
f_vwap_session_open(string sessionTimezone, int openingHour, int openingMinute) =>
    int openingTime = timestamp(
         sessionTimezone,
         ...
         openingHour,
         openingMinute)
    time >= openingTime and (na(time[1]) or time[1] < openingTime)

bool tokyoVwapOpen = f_vwap_session_open("Asia/Tokyo", 9, 0)
bool londonVwapOpen = f_vwap_session_open("Europe/London", 8, 0)
bool newYorkVwapOpen = f_vwap_session_open("America/New_York", 9, 30)
bool resetSessionVwap = barstate.isfirst or tokyoVwapOpen or londonVwapOpen or newYorkVwapOpen
float sessionVwapValue = ta.vwap(hlc3, resetSessionVwap)

plot(
     showSessionVwapInput and inGlobalRenderWindow ? sessionVwapValue : na,
     title = "VWAP Session",
     color = vwapColor,
     linewidth = 2)

var label sessionVwapLabel = na

if showSessionVwapInput and insightsAllowed and not na(sessionVwapValue)
    if na(sessionVwapLabel)
        sessionVwapLabel := label.new(
             bar_index,
             sessionVwapValue,
             "VWAP Session",
             style = label.style_label_left,
             ...
             textcolor = vwapColor,
             size = size.small)
    label.set_x(sessionVwapLabel, bar_index)
    label.set_y(sessionVwapLabel, sessionVwapValue)
else if not na(sessionVwapLabel)
    label.delete(sessionVwapLabel)
    sessionVwapLabel := na
...
plot(
     showEmasInput and inGlobalRenderWindow ? fastEma : na,
     title     = "Fast EMA",
     color     = emaFastColor,
     linewidth = 2)

plot(
     showEmasInput and inGlobalRenderWindow ? slowEma : na,
     title     = "Slow EMA",
     color     = emaSlowColor,
     linewidth = 2)
...
if showEmasInput and insightsAllowed and na(emaFastLabel) and not na(fastEma)
...
if showEmasInput and insightsAllowed and na(emaSlowLabel) and not na(slowEma)
