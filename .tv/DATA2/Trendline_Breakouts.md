...
//    - The chart scale (Regular or Logarithmic) is a setting. Match it to the
//      scale of your chart, otherwise the drawn lines will not sit on the
//      pivots they were built from.
//    - This build scans bullish structure only.
//
//  Version history
//    1.1  (2026-09-16)
//         - New "Chart scale" input at the top of group 1: Regular or
//           Logarithmic, same names TradingView uses on the price axis. The
//           engine already ran through f_scale / f_unscale, so the only change
//           to the maths is that "Logarithmic" now reaches them. On a log scale
//           the slope, the touch check and the break check are all done on
//           log(price), so a straight line on a log chart is treated as a
//           straight line.
//         - The old read-only "Price scale" marker in group 4 is gone; the
//           switch replaces it.
//         - The panel now shows which chart scale the maths is using.
//         - Header comments updated (limitations, scope section).
//    1.0  Initial public build. Linear price scale only, bullish structure
//         only, visible range recompute, summary panel.
// =============================================================================
...
scale_mode        = input.string("Regular", title = "Chart scale",                       options = ["Regular", "Logarithmic"], group = G_TREND, tooltip = "Pick the same scale your chart is on (right click the price axis to see it). Regular: a straight line joins equal price steps. Logarithmic: a straight line joins equal percentage steps. If the two do not match, the drawn lines will not sit on the pivots they were built from.")
...
// single value, so they are read-only markers rather than switches. The chart
// scale switch lives in group 1 because it changes the maths.
...
// agree on which scale a straight line means. Line endpoints are still passed
// to line.new in price units: the chart itself takes care of drawing them
// straight on whichever scale it is showing.
...
if barstate.islast
    ...
    if show_panel
        ...
        table.cell(panel, 0, 5, "Chart scale (v" + VERSION + ")", text_color = color.silver, text_size = size.small, text_halign = text.align_left)
        table.cell(panel, 1, 5, use_log ? "Logarithmic" : "Regular", text_color = color.white, text_size = size.small)
````
