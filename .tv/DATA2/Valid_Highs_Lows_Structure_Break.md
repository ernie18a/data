...
if mode == 1 and not na(lastPL) and not na(runHigh) and (useClose ? close < lastPL : low < lastPL)
    VH   := runHigh
    mode := 2
    if showPts
...
if mode == 2 and not na(lastPH) and not na(runLow) and (useClose ? close > lastPH : high > lastPH)
    VL   := runLow
    mode := 1
    if showPts
