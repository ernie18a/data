...
// --- Inputs ---
price_sel = input.string("close", "Price Mode?", options=["close", "hlcc4", "hl2", "hlc3"], group= "🔔 Sound Alerts")
ema_len = input.int(9, "MME Reactor Core", minval=1, group= "🔔 Sound Alerts")
...
// --- Plot columns ---
plot(pvt_ema, style=plot.style_columns, color=color_pvt, linewidth=4, title="Volume Bull / Bear")

// --- Dynamic background ---
bgcolor(barstate.isconfirmed ? (pvt_change >= 0 ? color.new(color.blue, 75) : color.new(color.orange, 75)) : na, title="BG Color Bull / Bear")
...
// --- Real Time ---
volume_bull_realtime = barstate.isrealtime and pvt_change > 0
volume_bear_realtime = barstate.isrealtime and pvt_change <= 0

// --- Confirmed ---
volume_bull_confirmed = barstate.isconfirmed and pvt_change > 0
volume_bear_confirmed = barstate.isconfirmed and pvt_change <= 0

// --- Alert conditions ---
alertcondition(volume_bull_realtime, title="Volume Bull — Real Time", message="Volume: Bullish pressure — Real Time")
alertcondition(volume_bear_realtime, title="Volume Bear — Real Time", message="Volume: Bearish pressure — Real Time")
alertcondition(volume_bull_confirmed, title="Volume Bull — Confirmed", message="Volume: Bullish pressure — Confirmed")
alertcondition(volume_bear_confirmed, title="Volume Bear — Confirmed", message="Volume: Bearish pressure — Confirmed")
````
