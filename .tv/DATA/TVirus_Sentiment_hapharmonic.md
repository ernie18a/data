...
Source: https://www.tradingview.com/script/jpbq3J0S-T-Virus-Sentiment-hapharmonic/
...
[image]https://www.tradingview.com/x/NpNNhgzH/[/image]
...
This is not just another line on a chart. It's a comprehensive sentiment dashboard designed to give an immediate, at-a-glance understanding of the confluence between 7 classic technical indicators. The incredible 3D model of the vial itself was inspired by a design concept found [here](https://blenderartists.org/t/tyrant-virus-resident-evil/588684).
...
[image]https://www.tradingview.com/x/SmK07GIO/[/image]
...
[image]https://www.tradingview.com/x/eGmJruUr/[/image]
...
[image]https://www.tradingview.com/x/xFzD1Msv/[/image]
...
[image]https://www.tradingview.com/x/xaWuzLkW/[/image]
...
[image]https://www.tradingview.com/x/xfjw4cKX/[/image]
...
group_sig = '⚙️ Signals (Placement into 5 Tube Levels)'
...
float obvSpikeKInput    = input.float(1.8, 'Spike k*avg(|Δ|)', minval=0.5, maxval=5.0, step=0.1, group = group_obv, tooltip='The multiplier for the average absolute change in OBV to detect a volume spike.', active=showSignalsInput, display= dis)
...
f_groupRSI_ex(float r, int len, float src) =>
    ...
    string why = switch grp
        1 => 'RSI Group 1 — Base Bottom\n• RSI > 80 (Overbought extreme)\n• Weakness expected\nFormula: RSI(' + str.tostring(len) + ', source)'
        2 => 'RSI Group 2 — Lower Tube\n• 70 < RSI ≤ 80 (Overbought, easing)\nFormula: RSI(' + str.tostring(len) + ', source)'
        3 => 'RSI Group 3 — Middle Tube\n• 30 ≤ RSI ≤ 70 (Neutral)\nFormula: RSI(' + str.tostring(len) + ', source)'
        4 => 'RSI Group 4 — Upper Tube\n• RSI < 30 (Oversold)\nFormula: RSI(' + str.tostring(len) + ', source)'
        5 => 'RSI Group 5 — Base Top\n• RSI < 20 (Deep Oversold, bullish potential)\nFormula: RSI(' + str.tostring(len) + ', source)'
        => ''
    [grp, why]
f_groupEMA_ex(float e9,float e21,float e50,float e100,float e200,float e250) =>
    ...
    string why = switch grp
        1 => 'EMA Group 1 — Base Bottom\n• Strict down-sequence: 9<21<50<100<200<250'
        2 => 'EMA Group 2 — Lower Tube\n• Early cross: 9>21 but 21<50'
        3 => 'EMA Group 3 — Middle Tube\n• Partial up-sequence: 9>21>50 but 50<100'
        4 => 'EMA Group 4 — Upper Tube\n• 9>21>50>100 (200/250 not fully aligned)'
        5 => 'EMA Group 5 — Base Top\n• Full up-sequence: 9>21>50>100>200>250'
        => ''
    [grp, why]
f_groupMACD_ex(float macd, float signal, float hist, int fLen, int sLen, int sigLen , srcMA , sigMA) =>
    ...
    string why = switch grp
        1 => 'MACD Group 1 — Base Bottom\n• macd<0, hist<0 and falling(hist,3)\nMomentum negative accelerating'
        2 => 'MACD Group 2 — Lower Tube\n• macd<0 with hist≥0 or rising(hist,3)\nNegative but improving'
        3 => 'MACD Group 3 — Middle Tube\n• Cross events near zero (transition)'
        4 => 'MACD Group 4 — Upper Tube\n• macd>0, hist>0 with macd>signal or rising(hist,3)'
        5 => 'MACD Group 5 — Base Top\n• macd>0, hist>0 with rising(macd,5) & rising(hist,3)'
        => ''
...
f_groupOBV_ex(float obv, float obvMa, int riseShort, int riseLong, int fallShort, float delta, float absDeltaAvg, float kSpike, float nearTolPct , maType, int maLen, int spikeLen) =>
    ...
    string why = 'OBV Group ' + str.tostring(grp) + '\n• OBV vs MA: ' + (isAbove ? 'Above' : isBelow ? 'Below' : 'Near') + '\n• ΔOBV=' + str.tostring(delta, '#.##') + ', avg|Δ|(' + str.tostring(spikeLen) + ')=' + str.tostring(absDeltaAvg, '#.##') + (spikeBuy ? ' [Spike]' : '') + '\n• rising(' + str.tostring(riseShort) + '/' + str.tostring(riseLong) + '), falling(' + str.tostring(fallShort) + ')\nFormula: OBV cum(sign(ΔClose)*Vol), MA=' + maType + '(' + str.tostring(maLen) + '), NearTol=' + str.tostring(nearTolPct) + ', Spike k=' + str.tostring(kSpike)
    [grp, why]
