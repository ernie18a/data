<!-- tradingview-pine-id: PUB;df964eb9904143eb88fe82e48d22b9cc -->
<!-- tradingviewscripts-format: 1 -->
# Supertrend with Entry, TP1, TP2 and TP3

Source: https://www.tradingview.com/script/njRv2ENC-Supertrend-with-Entry-TP1-TP2-and-TP3/

## Description

This is the strategy version of the LuxAlgo Quant Supertrend with Entry indicator. For backtesting purposes. I've found 5m to be the most successful, but I need to play with it a bit more and try to make it more than 50% profitable!
...
//@version=6
strategy("Supertrend with Entry, TP1, TP2 and TP3", "Supertrend Strategy", overlay = true, initial_capital = 10000, default_qty_type = strategy.percent_of_equity, default_qty_value = 100, commission_type = strategy.commission.cash_per_contract, commission_value = 0, slippage = 0)
...
tp1PercentInput = input.float(50, "TP1 Exit %", minval = 0, maxval = 100, step = 5, tooltip = "Percent of the position closed at TP1.", group = "Trade Levels")
tp2PercentInput = input.float(25, "TP2 Exit %", minval = 0, maxval = 100, step = 5, tooltip = "Percent of the ORIGINAL position closed at TP2.", group = "Trade Levels")
useDateFilterInput = input.bool(false, "Limit Backtest Date Range", tooltip = "Restrict trades to the date range below.", group = "Backtest Window")
startDateInput = input.time(timestamp("2026-01-01 00:00"), "Start Date", group = "Backtest Window")
endDateInput = input.time(timestamp("2026-12-31 23:59"), "End Date", group = "Backtest Window")
...
inDateWindow = not useDateFilterInput or (time >= startDateInput and time <= endDateInput)
...
if sellSignal and inDateWindow
    entryLevel := close
    stopLossLevel := entryLevel + atrValue * slAtrMultiplierInput
...
if strategy.position_size > 0
    strategy.exit("Long TP1/SL", from_entry = "Long", stop = stopLossLevel, limit = takeProfit1Level, qty_percent = tp1PercentInput)
    strategy.exit("Long TP2/SL", from_entry = "Long", stop = stopLossLevel, limit = takeProfit2Level, qty_percent = tp2PercentInput)
    strategy.exit("Long TP3/SL", from_entry = "Long", stop = stopLossLevel, limit = takeProfit3Level)

if strategy.position_size < 0
    strategy.exit("Short TP1/SL", from_entry = "Short", stop = stopLossLevel, limit = takeProfit1Level, qty_percent = tp1PercentInput)
    strategy.exit("Short TP2/SL", from_entry = "Short", stop = stopLossLevel, limit = takeProfit2Level, qty_percent = tp2PercentInput)
    strategy.exit("Short TP3/SL", from_entry = "Short", stop = stopLossLevel, limit = takeProfit3Level)
