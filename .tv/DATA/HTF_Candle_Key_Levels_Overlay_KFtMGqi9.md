<!-- tradingview-pine-id: PUB;57b4c95609e048ff829b7d4aad354f8a -->
<!-- tradingviewscripts-format: 1 -->
# HTF Candle & Key Levels Overlay

Source: https://www.tradingview.com/script/KFtMGqi9-HTF-Candles-Key-Levels/
...
right_offset = input.int(4, "Abstand der rechten Kerze (Bars)", minval=1, group=group_htf)
candle_width = input.int(3, "Breite der rechten Kerze (Bars)", minval=1, group=group_htf)
...
// Fib Toggles (Standards exakt nach Vorgabe)
show_1    = input.bool(false, "100.0% (High)", inline="f1", group=group_fib)
...
show_0    = input.bool(false, "0.0% (Low)", inline="f0", group=group_fib)

// Line Customization
line_style_str = input.string("Solid", "Linienstil", options=["Solid", "Dashed", "Dotted"], group=group_fib)
fib_color      = input.color(#636363, "Haupt-Linienfarbe", group=group_fib)
...
// Background Zones Customization
...
up_color    = input.color(#0cc513, "Bullish Body", group=group_color)
down_color  = input.color(color.black, "Bearish Body", group=group_color)
wick_color  = input.color(#636363, "Docht Farbe", group=group_color)
wick_width  = input.int(1, "Docht Stärke", minval=1, maxval=5, group=group_color)
...
add_fib_level(float price, string level_text, color l_color, int l_width, color txt_color_input, bool is_50_level) =>
    if show_fib
        ...
        // Kleine Verbindungslinie bei der HTF-Kerze
        if draw_right
...
// Positionen der rechten Kerze
int right_x1 = bar_index + right_offset
int right_x2 = right_x1 + (candle_width - 1)

// Exakte Mitte für den Docht
...
// Hintergrund-Boxen enden an der aktuellen Kerze
if show_bg_40_50
