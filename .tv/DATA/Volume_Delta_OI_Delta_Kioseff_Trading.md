...
Source: https://www.tradingview.com/script/xp9OdTFm-Volume-Profile-Volume-Delta-OI-Delta-Kioseff-Trading/
...
[image]https://www.tradingview.com/x/GW8ChVA5/[/image]

The image above provides further explanation of functionality and color correspondence. 

[image]https://www.tradingview.com/x/ykz7IDso/[/image]
...
[image]https://www.tradingview.com/x/qmUXYLbY/[/image]
...
[image]https://www.tradingview.com/x/OB1j3RSg/[/image]

The image above shows a heatmap using volume delta calculations.

[image]https://www.tradingview.com/x/SGcIYCKM/[/image]
...
c                                       = input.color(defval = #55ffda, title = 'General "Up/Buy" Color', group = 'Up/Buy Color')
c1                                      = input.color(defval = color.rgb(255, 116, 116), title = 'General "Down/Sell" Color', group = 'Down/Sell Color')
c2                                      = input.color(defval = #ddff55, title = 'General "POC" Color', group = '"POC" Color')
...
colorChange(x, y) => 
    ...
    label.set_textcolor(x, y)
    label.set_color(x, color.new(y, 90))
    ...
    if array.size(boxCo) == 1 
        ...
        if array.size(lettersBox) > 0 or array.size(lettersBoxClassic) > 0
            ...
            if classic == false 
                ...
                colorChange(array.get(lettersBox, 0), array.sum(volCountUp) > array.sum(volCountDn) ? c : c1) 
...
if data == "OI Delta (BINANCE PERP FUTURES ONLY)" 
    ...
    if array.size(boxCo) == 1 and array.size(m2) == array.size(clo)
        ...
        if array.size(lettersBox) > 0 or array.size(lettersBoxClassic) > 0
            ...
            if classic == false 
                label.set_text(array.get(lettersBox, 0), cache)
                colorChange(array.get(lettersBox, 0), array.sum(volCountUp) + array.sum(volCountDn) >= 0 ? c : c1) 
            else 
                box.set_text(array.get(lettersBoxClassic, 0), cache)
