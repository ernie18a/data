...
Source: https://www.tradingview.com/script/lJXwpXtt-ZigzagLite/
...
// @field sizeRatio ratio of current zigzag wave size in comparison to last zigzag wave in the same direction
...
// @field force_overlay force drawing in price overlay
...
method clear(ZigzagDrawingPL this) =>
...
// @returns ZigzagDrawing object
export method drawfresh(ZigzagDrawing this, array<float> ohlc = na) =>
    this.init()
    this.clear()
    this.zigzag.calculate(na(ohlc) ? array.from(high, low) : ohlc)
    this.drawplain()
...
// @param ohlc values on which the zigzag needs to be calculated and drawn. If not set will use regular OHLC
// @returns [array<line> zigzaglines, array<label> zigzaglabels]
export method drawcontinuous(ZigzagDrawing this, array<float> ohlc = na) =>
    this.init()
    this.zigzag.calculate(na(ohlc) ? array.from(high, low) : ohlc)
