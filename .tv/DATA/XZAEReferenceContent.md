...
export const string TT_SHOW_TIGHTEST_PA_BRACKET = "Presentation-only override. Shows the nearest eligible retained validated-Range quartile below and above live Price Action (PA), while explicit Focused R#/C# objects remain available as the study layer. Other automatic R#/C#/L#, waiting-origin and global pivot-audit geometry is suppressed. NPA renders last so exact price coincidences resolve to one NPA line/tag, and its tag carries full R#/level provenance when Chart Audit Tooltips are enabled. Archived eligibility follows Hide Visually Archived Ranges; Max Validated Ranges Displayed = 0 disables NPA. Analytical state is unchanged."
...
export const string TT_ZONE_TAG_DISPLAY_MODE = "Audit Tag is the default compact ownership surface at the right/end of each displayed S&D zone: e.g. R#35 · D or C#47 · S. The tag identifies the displayed-through object and zone side; when Chart Audit Tooltips are enabled, hovering that same tag reports canonical S&D origin plus all retained object relationships using that Swing-associated zone. Compact Text keeps the larger two-line ownership decoder at the object origin. Off removes zone tags."
export const string TT_SHOW_QUARTILE_LABELS = "Shows labels on every currently focused Range and Candidate. The structural 0% and 100% endpoint tags carry the owning R#/C# ID and act as the forensic hover surface when Chart Audit Tooltips are enabled. 25/50/75 remain pure geometry decoders. Non-focus context keeps its geometry without competing label stacks."
...
export const string ST_SOURCE_TIP = "Auction Source Timeframe. The source registry owns analytical identity within the 100,000-source-bar replay budget; host chart timeframe is presentation-only within the supported Source ≥ Chart domain."
...
export const string ST_DISPLAY_LABEL_TIP = "Current chart allocation before geometry dedup/Nearest-PA presentation overrides. Allocation never creates, deletes, validates or otherwise changes analytical objects."
export const string ST_FOCUS_LABEL_TIP = "Explicit visual focus IDs. Focus is presentation state only."
...
export const string ST_SESSION_LABEL_TIP = "Selected canonical session context. Each session row uses its current developing occurrence when active, otherwise its most recently completed retained occurrence, and relates that H-L to operational AE geometry. Descriptive only; never participates in AE/MS/S&D authority."

export sessionStatusTooltip(string standard, string contextText) =>
    "Standard: " + standard + "\nContext: " + contextText + "\nClock authority: XZ Session Authority\nNo effect on C#/R#/L#, qualification, lifecycle, MS/S&D authority or Range selection."
//==============================================================================
...
export const string TT_SESSION_SHOW_CODES = "Shows the compact canonical session code on each displayed Session High-Low frame (SYD, TYO, LDN, NY)."
