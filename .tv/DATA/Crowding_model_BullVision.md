...
Source: https://www.tradingview.com/script/S9KNr6dG-Crowding-model-BullVision/
...
//@version=6
indicator(" Crowding model ║ BullVision",shorttitle = "👥 Crowding Model ║ BullVision👥", overlay=false)

// ==================== INPUTS ====================
group1 = '🎨 Visual Settings'
...
scatter(float rsiValue, float zScore, color clr, string symbol, int scale, string category = 'other') =>
    if barstate.islast
        ...
        emoji = rsiValue > 80 ? '🔥' : rsiValue < 20 ? '❄️' : ''
...
if showCrowdingPanel
    ...
    if barstate.islast
        // Title
        table.cell(panel, 0, 0, '🎯 CROWDING ANALYTICS', text_color = #58a6ff, text_size = size.normal, bgcolor = color.new(#161b22, 50))
        ...
        table.cell(panel, 0, 4, '🔥 FOMO', text_color = #ff4444, text_size = size.small)
        ...
        table.cell(panel, 0, 5, '📈 Recovery', text_color = #ffa500, text_size = size.small)
        ...
        table.cell(panel, 0, 6, '❄️ Fear', text_color = #2196f3, text_size = size.small)
        ...
        table.cell(panel, 0, 7, '💪 Strength', text_color = #00ff88, text_size = size.small)
...
if barstate.isconfirmed
    if fomoPct >= crowdingThreshold
        alert("🔥 Crowding Alert: FOMO levels have reached " + str.tostring(math.round(fomoPct, 1)) + "%.", alert.freq_once_per_bar_close)
    if fearPct >= crowdingThreshold
        alert("❄️ Crowding Alert: Fear levels have reached " + str.tostring(math.round(fearPct, 1)) + "%.", alert.freq_once_per_bar_close)
    if recoveryPct >= crowdingThreshold
        alert("📈 Crowding Alert: Recovery levels have reached " + str.tostring(math.round(recoveryPct, 1)) + "%.", alert.freq_once_per_bar_close)
    if strengthPct >= crowdingThreshold
        alert("💪 Crowding Alert: Strength levels have reached " + str.tostring(math.round(strengthPct, 1)) + "%.", alert.freq_once_per_bar_close)
````
