<!-- tradingview-pine-id: PUB;a149b39029d040e09abff70bfb27eb6b -->
<!-- tradingviewscripts-format: 1 -->
# MACD Divergence Pro [NQ]

Source: https://www.tradingview.com/script/dXrt2Sdc-MACD-Divergence-Pro/

## Description

A really simple macd divergence and continuation indicator that plot lines on the chart and the macd.
...
// Regular + Hidden divergence detection w/ strength scoring, dual MACD continuation signals (vs. 1 pivot back AND 2 pivots back), dual-pane drawing, alerts
    ...
    if showContBull2 and n >= 3
        ...
        if dist2 >= minLookback and dist2 <= maxLookback * 2 and score2 >= minStrengthToShow and (curFlip - farFlip) >= minHistFlips2
            if curPrice > farPrice and curMacd > farMacd
                ...
                if showLabels
                    label.new(curBar, curMacd, f_labelText("Cont2", score2), style=label.style_label_up, color=colContBull2, textcolor=color.white, size=size.tiny)
                if drawOnPriceChart
                    ...
                    if showLabels
                        label.new(curBar, curPrice, f_labelText("Cont2", score2), style=label.style_label_up, color=colContBull2, textcolor=color.white, size=size.tiny, force_overlay=true)
    ...
    if showContBear2 and n >= 3
        ...
        if dist2 >= minLookback and dist2 <= maxLookback * 2 and score2 >= minStrengthToShow and (curFlip - farFlip) >= minHistFlips2
            if curPrice < farPrice and curMacd < farMacd
                ...
                if showLabels
                    label.new(curBar, curMacd, f_labelText("Cont2", score2), style=label.style_label_down, color=colContBear2, textcolor=color.white, size=size.tiny)
                if drawOnPriceChart
                    ...
                    if showLabels
                        label.new(curBar, curPrice, f_labelText("Cont2", score2), style=label.style_label_down, color=colContBear2, textcolor=color.white, size=size.tiny, force_overlay=true)
