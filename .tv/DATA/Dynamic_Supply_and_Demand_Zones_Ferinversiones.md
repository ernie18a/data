<!-- tradingview-pine-id: PUB;97bec8f263bd40e8ae255aeca8de0d58 -->
<!-- tradingviewscripts-format: 1 -->
# Dynamic Supply and Demand Zones [Fer_inversiones]

Source: https://www.tradingview.com/script/DkgUbjvW/

## Description

=== ENGLISH DESCRIPTION ===

This indicator automatically identifies and plots dynamic Supply and Demand zones on the chart using ATR-based buffer heights and volume weighting to highlight institutional order block areas.

### How it Works:
1. Swing High / Swing Low Detection: Uses pivot highs and lows to identify potential liquidity turning points.
2. Volume Filter & Ratio Calculation: Evaluates buying vs. selling volume ratios on the pivot candle to scale the zone's width and strength.
3. ATR Dynamic Sizing: Calculates zone height based on Average True Range (ATR) multiplied by the volume intention percentage.
4. Mitigation & Retest Tracking: Tracks when price retests a zone. Once price breaks past the distal border, the zone is mitigated and removed.
5. On-Screen Price Table: Displays the closest active Supply and Demand levels right on the chart.

### UI Translation (User Interface Settings):
- "Precio a mostrar" (Price to display):
  - Entrada (Proximal) = Entry / Proximal Price
  - 50% (Equilibrium) = 50% Equilibrium Level
  - Límite Opuesto (Distal) = Opposite / Distal Limit
- "Posición del Panel" (Table Position): Options for screen positioning (Top/Middle/Bottom - Left/Center/Right).

---

=== DESCRIPCIÓN EN ESPAÑOL ===

Este indicador identifica y dibuja automáticamente zonas dinámicas de Oferta (Supply) y Demanda (Demand) en el gráfico, utilizando rangos basados en ATR y ponderación de volumen para resaltar áreas de bloques de órdenes.

### Funcionalidades clave:
1. Detección de Pivotes (Swings): Identifica máximos y mínimos claves.
2. Filtro de Volumen: Calcula el porcentaje de intención compradora/vendedora.
3. Altura Dinámica por ATR: Ajusta el grosor de las zonas según la volatilidad y el volumen de la vela de origen.
4. Control de Re-tests: Cambia el color cuando las zonas son testeadas y las elimina cuando son invalidadas/mitigadas.
5. Tabla de Precios: Muestra en pantalla los niveles más cercanos de Supply y Demand en tiempo real.
...
````pine
//@version=6
indicator(title='Dynamic Supply and Demand Zones [Fer_inversiones]', shorttitle="Sop/Res DINÁMICO", overlay=true, max_bars_back=500)
