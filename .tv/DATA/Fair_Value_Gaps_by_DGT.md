...
Source: https://www.tradingview.com/script/Onab2fvE-Fair-Value-Gaps-by-DGT/
...
Detect and display FVGs from the current chart, higher timeframes (HTF), or lower timeframes (LTF)[image]https://www.tradingview.com/x/5LNx1wiy/[/image]

⯌ Smart Fill Tracking
Automatic real-time monitoring of each FVG’s fill progress with live percentage updates[image]https://www.tradingview.com/x/caSKOoP4/[/image]

⯌ Custom Fill Logic
Choose your preferred definition of when a gap is considered filled:[*]Any Touch
[*]Midpoint Reached
[*]Wick Sweep
[*]Body Beyond[image]https://www.tradingview.com/x/nGbFzTGY/[/image]

⯌ Dynamic Labels & Tooltips
Labels can be toggled on/off. Even when hidden, detailed tooltips remain available by hovering over the FVG midpoint.[image]https://www.tradingview.com/x/nRcR03KC/[/image]

⯌ Adaptive Lower-Timeframe Mode
When set to “Auto,” the script intelligently selects the optimal lower timeframe based on the chart resolution.[image]https://www.tradingview.com/x/XpeflKh8/[/image]
...
fvgTF3 = input.string('Auto', '', 
    options = ['Auto', '1S', '5S', '10S', '15S', '30S', '1m', '3m', '5m', '15m', '30m', '45m', '1H', '2H', '3H', '4H', 'D'], inline = 'fvg3', display = display, group = fvgGR3, tooltip = fvgTip3)
...
fvgTH3 = input.float(.7, '  FVG Filter Threshold', minval = 0, step = .1, group = fvgGR3, tooltip = fvgThTip, display = display)
    ...
    if show
        ...
        if bullCondition and not bullProcess
            ...
            bullFvgArray.unshift(
             FVG.new(
                 label.new(bar.index + 1, math.avg(barArray.get(0).low.price, barArray.get(2).high.price), fvgLb == 'Show' ? fvgTf ? 'Chart(' + tf + ') FVG' : 'HTF(' + tf + ') FVG' : '', xloc.bar_index, color = noColor, style = label.style_label_left, textcolor = cBull, size = size.small, 
        ...
        if bearCondition and not bearProcess
            ...
            bearFvgArray.unshift(
             FVG.new(
                 label.new(bar.index + 1, math.avg(barArray.get(2).low.price, barArray.get(0).high.price), fvgLb == 'Show' ? fvgTf ? 'Chart(' + tf + ') FVG' : 'HTF(' + tf + ') FVG' : '', xloc.bar_index, color = noColor, style = label.style_label_left, textcolor = cBear, size = size.small, 
...
if fvgSH3 and timeframe.change(timeframe.period) and array.size(aH) > 0 
    ...
    for i = 0 to aH.size() - 1 by 1
        ...
        if bull
            ...
            fvgLTFlb.push(
             label.new(bar.index + 1, math.avg(bar.ltfLow, bar.ltfHigh2), fvgLB3 == 'Show' ? 'LTF(' + ltf + ') FVG' : '', xloc.bar_index, color = noColor, style = label.style_label_left, textcolor = fvgCb3, size = size.small, 
        ...
        if bear
            ...
            fvgLTFlb.push(
             label.new(bar.index + 1, math.avg(bar.ltfLow2, bar.ltfHigh), fvgLB3 == 'Show' ? 'LTF(' + ltf + ') FVG' : '', xloc.bar_index, color = noColor, style = label.style_label_left, textcolor = fvgCs3, size = size.small, 
