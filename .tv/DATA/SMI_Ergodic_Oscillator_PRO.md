...
//@version=6
indicator(title="SMI Ergodic Oscillator PRO", shorttitle="SMIO PRO", format=format.price, precision=4)

// === INPUTS ===

longlen  = input.int(20, minval=1, title="Long Length")
shortlen = input.int(5, minval=1, title="Short Length")
siglen   = input.int(5, minval=1, title="Signal Length")

// === MA TYPE ===

maType = input.string("EMA", "Signal MA Type", options=["EMA", "SMA", "RMA"], group="🔔 Sound Alerts")

// === SOURCE ===

srcOpt = input.string("close", "Source", options=["close", "hl2", "hlc3", "hlcc4", "ohlc4"], group="🔔 Sound Alerts")
