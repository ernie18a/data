...
Source: https://www.tradingview.com/script/hVn0eEko-Multi-Anchored-Linear-Regression-Channels-TANHEF/
...
[image]https://www.tradingview.com/x/hlWWE7J5/[/image]
...
[image]https://www.tradingview.com/x/OAWdcmeo/[/image]
...
[image]https://www.tradingview.com/x/RwBUH7Y6/[/image]
⯁ Selecting Bias:
...
[image]https://www.tradingview.com/x/9ZmAL7Kf/[/image]
⯁ Scale Awareness:

[*]Supports linear and logarithmic price scaling, the regression and deviations adjust accordingly.

[image]https://www.tradingview.com/x/0cwgspsv/[/image]
...
[image]https://www.tradingview.com/x/q6Hdprhq/[/image]
...
[image]https://www.tradingview.com/x/I6z94ARz/[/image]
...
[image]https://www.tradingview.com/x/toJ22gI9/[/image]
...
[image]https://www.tradingview.com/x/ETKUyEka/[/image]
...
inputMax1 = input.int(1000, title = 'Length│Max', inline = 'len1b', minval = 2, maxval = max_bars, group = g_reg1, active = inputDisplay1 != DisplayLinReg.Hide)
...
inputRegType1 = input.enum(RegressionType.Standard, title = 'Regression│Type', inline = 'len1c', group = g_reg1, tooltip = tt_reg, active = inputDisplay1 != DisplayLinReg.Hide)
...
inputDevType1 = input.enum(DeviationType.Reverse, title = 'Deviation│Type', inline = 'len1d', tooltip = tt_dev, group = g_reg1, active = inputDisplay1 != DisplayLinReg.Hide)
...
inputDev1 = input.float(2, title = '✕', inline = 'len1d', minval = 0.1, step = 0.1, group = g_reg1, active = inputDisplay1 != DisplayLinReg.Hide)
...
inputMax2 = input.int(1000, title = 'Length│Max', inline = 'len2b', minval = 2, maxval = max_bars, group = g_reg2, active = inputDisplay2 != DisplayLinReg.Hide)
...
inputRegType2 = input.enum(RegressionType.Standard, title = 'Regression│Type', inline = 'len2c', group = g_reg2, tooltip = tt_reg, active = inputDisplay2 != DisplayLinReg.Hide)
...
inputDevType2 = input.enum(DeviationType.Reverse, title = 'Deviation│Type', inline = 'len2d', tooltip = tt_dev, group = g_reg2, active = inputDisplay2 != DisplayLinReg.Hide)
...
inputDev2 = input.float(2, title = '✕', inline = 'len2d', minval = 0.1, step = 0.1, group = g_reg2, active = inputDisplay2 != DisplayLinReg.Hide)
...
inputMax3 = input.int(400, title = 'Length│Max', inline = 'len3b', minval = 2, maxval = max_bars, group = g_reg3, active = inputDisplay3 != DisplayLinReg.Hide)
...
inputRegType3 = input.enum(RegressionType.Standard, title = 'Regression│Type', inline = 'len3c', group = g_reg3, tooltip = tt_reg, active = inputDisplay3 != DisplayLinReg.Hide)
...
inputDevType3 = input.enum(DeviationType.Reverse, title = 'Deviation│Type', inline = 'len3d', tooltip = tt_dev, group = g_reg3, active = inputDisplay3 != DisplayLinReg.Hide)
...
inputDev3 = input.float(2, title = '✕', inline = 'len3d', minval = 0.1, step = 0.1, group = g_reg3, active = inputDisplay3 != DisplayLinReg.Hide)
...
dirUpPercentAVG = input.enum(SignalPercent.Any, title = 'ᴬᵛᵍ', inline = 'AVGup1', group = g_signalsGreen, active = inputDisplay1 != DisplayLinReg.Hide or inputDisplay2 != DisplayLinReg.Hide or inputDisplay3 != DisplayLinReg.Hide)
...
dirDnPercentAVG = input.enum(SignalPercent.Any, title = 'ᴬᵛᵍ', inline = '1AVGdown1', group = g_signalsRed, active = inputDisplay1 != DisplayLinReg.Hide or inputDisplay2 != DisplayLinReg.Hide or inputDisplay3 != DisplayLinReg.Hide)
...
displaySelected = input(true, title = 'Input - Displayed (#1,#2,#3,Average)', group = g_table, active = displayTable, tooltip = '█ Displayed (#1,#2,#3,Average)\nDisplayed linear regressions.')
displayInputAnchor = input(true, title = 'Input - Anchor', group = g_table, active = displayTable, tooltip = '█ Anchor\nStarting point type (anchor) for regression.')
displayInputLength = input(true, title = 'Input - Length (Max,Min,Step)', group = g_table, active = displayTable, tooltip = '█ Length (Max,Min,Step)\nLength criteria for regression.')
displayInputRegType = input(true, title = 'Input - Regression Type', group = g_table, active = displayTable, tooltip = '█ Regression Type\nFormula type for Linear Regression.')
displayInputRegBias = input(true, title = 'Input - Regression Bias', group = g_table, active = displayTable, tooltip = '█ Regression Bias\nEMA-Styled Emphasis Weighting.')
displayInputDevType = input(true, title = 'Input - Deviation Type', group = g_table, active = displayTable, tooltip = '█ Deviation Type\nFormula type for Deviations.')
displayInputDevBias = input(true, title = 'Input - Deviation Bias', group = g_table, active = displayTable, tooltip = '█ Deviation Bias\nEMA-Styled Emphasis Weighting.')
displayInputDev = input(true, title = 'Input - Deviation Multiplier', group = g_table, active = displayTable, tooltip = '█ Deviation Multiplier\nStandard deviation multiplier of linear regression.')
displayInputSignal = input(true, title = 'Input - Signal Criteria (Percent and R)', group = g_table, active = displayTable, tooltip = '█ Signal Criteria\nPercent and R criteria for signals. Shown if signal criteria selected.')
displayLength = input(true, title = 'Calculated - Length (LinReg)', group = g_table, active = displayTable, tooltip = '█ Length\nLength of LinReg calculated from the specified anchor and length inputs.')
displayPercent = input(true, title = 'Calculated - Percent (Price vs Deviations)', group = g_table, active = displayTable, tooltip = '█ Percent\n│￣100% (Top [+]Deviation)\n│－ 50% (Regression Line)\n│＿ 0% (Lower [-]Deviation)')
displayCloseDev = input(true, title = 'Calculated - Deviation of Close (ⓒ)', group = g_table, active = displayTable, tooltip = '█ Deviation value of current (close) price).')
displayR = input(true, title = 'Calculated - R (Pearson Correlation Coefficient)', group = g_table, active = displayTable, tooltip = '█ R (Pearson Correlation Coefficient)\nIndicating the strength and direction of the linear relationship between the data points.')
displayR2 = input(false, title = 'Calculated - R² (Determination Coefficient)', group = g_table, active = displayTable, tooltip = '█ R² (Determination Coefficient)\nIndicaties how well the regression predictions approximate the real data points.')
displayAverageAbsR = input(false, title = 'Calculated - │R│ Average (Pearson Correlation Coefficient)', group = g_table, active = displayTable, tooltip = '█ │R│ Average (Pearson Correlation Coefficient)\nAverages all │R│ values, finding average correlation strength of historical regressions.')
displaySlope = input(false, title = 'Calculated - Slope', group = g_table, active = displayTable, tooltip = '█ Slope\nValue of slope.')
displayTrend = input(false, title = 'Calculated - Trend', group = g_table, active = displayTable, tooltip = '█ Trend\nTrend direction.')
displayBarsSide = input(false, title = 'Calculated - Above/Below Regression (↑/↓)', group = g_table, active = displayTable, tooltip = '█ Above/Below Regression (↑/↓)\nCounts closes (or other data source) above and below the regression since the first plot, relative to the current bar\'s position on the line.\n(Counts last bar of each regression only)')
...
roundPercentValue = input.int(1, title = '', minval = 0, maxval = 30, inline = 'roundPercent', group = g_table, active = displayTable, tooltip = "█ Format percent type\n● 'Sig Figs' (significant figures)\n● 'Decimal' points.\n\n█ Format percent value\nValue applied to format.")
roundValueType = input.enum(RoundingValue.SigFigs, title = 'Format Value', inline = 'roundValue', active = displayTable, group = g_table)
roundValueValue = input.int(5, title = '', minval = 1, maxval = 30, inline = 'roundValue', group = g_table, active = displayTable, tooltip = "█ Format values type\n● 'Sig Figs' (significant figures)\n● 'Decimal' points.\n\n█ Format value\nValue applied to format.")
...
//CALCULATE - Deviation multiples (×) for close
calcDeviationMultiple(_topPrice, _botPrice, _priceTransformed, _devX) =>
    ...
    (_pct - 50.0) / 50.0 * _devX                                               // mid=0, top=+✕, bot=−✕
...
if barstate.islast
    ...
    if displayTable
        ...
        if displayInputDev
            createCell(r, 1, displayAny, 'Dev ✕', col_tblInputs, col_tblHeadingBG, 'Input, Deviation Multiplier')
        ...
        // Deviation of Close
        if displayCloseDev
            createCell(r, 1, displayAny, 'Dev ⓒ', col_tblCalculations, col_tblHeadingBG, 'Calculated, Deviation value at close.')
        ...
        // Pearson's R²
        if displayR2
            createCell(r, 1, displayAny, 'R²', col_tblCalculations, col_tblHeadingBG, 'Calculated, R-Squared (Determination Coefficient)')
        ...
        // Average Pearson's R
        if displayAverageAbsR
            createCell(r, 1, displayAny, 'Avg │R│', col_tblCalculations, col_tblHeadingBG, 'Calculated, Average │R│ (Pearson Correlation Coefficient)\n(1 = perfect correlation, 0 = none)')
        ...
        if displayTrend
            ...
            createCell(r, 2, show1, len1 == 1 ? "" : slope1 > 0 ? "↗" : (slope1 < 0 ? "↘" : "→"), col_tblCalculations, col_tblContentBG, "")
            createCell(r, 3, show2, len2 == 1 ? "" : slope2 > 0 ? "↗" : (slope2 < 0 ? "↘" : "→"), col_tblCalculations, col_tblContentBG, "")
            createCell(r, 4, show3, len3 == 1 ? "" : slope3 > 0 ? "↗" : (slope3 < 0 ? "↘" : "→"), col_tblCalculations, col_tblContentBG, "")
            createCell(r, 5, showAVG, slopeAVG > 0 ? "↗" : (slopeAVG < 0 ? "↘" : "→"), col_tblCalculations, col_tblContentBG, "")
            r += 1

        // Above/Below Regression (↑/↓)
        if displayBarsSide
            createCell(r, 1, displayAny, "↑/↓", col_tblCalculations, col_tblHeadingBG, "Calculated, counts above & below regression\n(Counts last bar of each regression only)")
