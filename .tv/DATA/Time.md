...
Source: https://www.tradingview.com/script/tyeeNU9I-Time/
...
formattedDay(timenow, "yyyy.MM.dd G 'at' hh:mm:ss z")   >>>     "2022.04.30 AD at 12:40:35 UTC"[/pine]

These functions use [str.format()](https://www.tradingview.com/pine-script-reference/v5/#fun_str{dot}format) and some of the special formatting codes it allows for. Pine Script™ documentation does not yet contain complete specifications on these codes, but in the meantime you can find some information in the [The Java™ Tutorials](https://docs.oracle.com/javase/tutorial/i18n/format/index.html) and in Java documentation of its [MessageFormat class](https://docs.oracle.com/javase/10/docs/api/java/text/MessageFormat.html). Note that [str.format()](https://www.tradingview.com/pine-script-reference/v5/#fun_str{dot}format) implements only a subset of the MessageFormat features in Java.

`secondsSince()`

The introduction of [varip](https://www.tradingview.com/pine-script-reference/v5/#op_varip) variables in Pine Script™ has made it possible to track the time for which a condition is true when a script is executing on a realtime bar. One obvious use case that comes to mind is to enable trades to exit only when the exit condition has been true for a period of time, whether that period is shorter that the chart's timeframe, or spans across multiple realtime bars.

For more information on this function and [varip](https://www.tradingview.com/pine-script-reference/v5/#op_varip) please see our [Using `varip` variables](https://www.tradingview.com/script/ppQxBISk-Using-varip-variables-PineCoders/) publication. 

`timeFrom( )`

When plotting [lines](https://www.tradingview.com/pine-script-reference/v5/#fun_line{dot}new), [boxes](https://www.tradingview.com/pine-script-reference/v5/#fun_box{dot}new), and [labels](https://www.tradingview.com/pine-script-reference/v5/#fun_label{dot}new) one often needs to calculate an offset for past or future end points relative to the time a condition or point occurs in history. Using [xloc.bar_index](https://www.tradingview.com/pine-script-reference/v5/#var_xloc{dot}bar_index) is often the easiest solution, but some situations require the use of [xloc.bar_time](https://www.tradingview.com/pine-script-reference/v5/#var_xloc{dot}bar_time). We introduce `timeFrom()` to assist in calculating time-based offsets. The function calculates a timestamp using a negative (into the past) or positive (into the future) offset from the current bar's starting or closing time, or from the current time of day. The offset can be expressed in units of chart timeframe, or in seconds, minutes, hours, days, months or years. This function was ported from our [Time Offset Calculation Framework](https://www.tradingview.com/script/5mZ7hV66-Time-Offset-Calculation-Framework-PineCoders-FAQ/).
...
How can I produce a "simple string" timeframe usable with [request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request{dot}security) from a timeframe expressed in seconds?
...
`secondsToTfString()` converts an input time in seconds to a target timeframe string in [timeframe.period](https://www.tradingview.com/pine-script-reference/v5/#var_timeframe{dot}period) string format. This is useful for implementing stepped timeframes relative to the chart time, or calculating multiples of a given chart timeframe. Results from this function are in [simple](https://www.tradingview.com/pine-script-reference/v5/#op_simple) form, which means they are useable as `timeframe` arguments in functions like [request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request{dot}security). 
